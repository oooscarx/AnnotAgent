...
export type TaskSchemaService=Pick<typeof api,"humanConversationSchemas"|"conversationSchemaCalls"|"conversationSchemaDraftForCall"|"conversationSchemaDraft"|"editConversationSchemaDraft">;
...
function SchemaList({service,project,conversation,task,workspace}:{service:TaskSchemaService;project:string;conversation:string;task:string;workspace:string}){
  ...
  return <section aria-label="任务语义草稿"><p>仅编辑此任务的标签语义草稿；不修改 Project Schema、Published Version 或正式标注，不调用模型。已有方案不会自动采用新 revision。</p>{error&&<p role="alert">{error}</p>}{!rows&&!error&&<p role="status">读取已保存草稿…</p>}{rows?.length===0&&<p>当前任务没有已保存的语义草稿。</p>}{rows?.map(d=><SchemaEdit key={d.id} initial={d} project={project} service={service} storageKey={`annotagent.schema-edit.${workspace}.${project}.${task}.${d.id}`}/>)}</section>;
}
type Edit=Parameters<TaskSchemaService["editConversationSchemaDraft"]>[2];
function SchemaEdit({initial,project,service,storageKey}:{initial:ConversationSchemaDraft;project:string;service:TaskSchemaService;storageKey:string}){
  ...
  const save=async()=>{
    if(lock.current||invalidRecovery)return;lock.current=true;setBusy(true);setError("");
  ...
  return <Disclosure title={`${draft.definition.task.labels.join("、")} · revision ${draft.revision}`}><p>{draft.definition.goal}</p><p role="status">{busy?"保存中…":pending?"保存结果待核实；只重试原请求":dirty?"有未保存修改":"已读取服务器草稿"}</p>
    <label>标签（每行一个）<textarea value={labels} disabled={busy||!!pending} onChange={e=>setLabels(e.target.value)}/></label><label>边界规则（每行一条）<textarea value={rules} disabled={busy||!!pending} onChange={e=>setRules(e.target.value)}/></label>
    {error&&<p role="alert">{error}。输入保留；未自动重新执行。</p>}<div className="actions"><button disabled={busy||!!pending||!dirty} onClick={()=>{setLabels(draft.definition.task.labels.join("\n"));setRules(draft.definition.boundary_rules.join("\n"));}}>取消修改</button><button disabled={invalidRecovery||busy||(!pending&&(!dirty||!labels.trim()))} onClick={()=>void save()}>{pending?"核实原保存请求":"保存语义草稿"}</button><button disabled={busy||!!pending||invalidRecovery} onClick={async()=>{if(dirty&&!window.confirm("丢弃当前编辑并读取最新草稿？"))return;setBusy(true);try{const v=await service.conversationSchemaDraft(project,draft.id);if(v.id!==draft.id||v.task_id!==draft.task_id)throw new Error("草稿归属不匹配");if(alive.current){setDraft(v);setLabels(v.definition.task.labels.join("\n"));setRules(v.definition.boundary_rules.join("\n"));setError("");}}catch(e){if(alive.current)setError((e as Error).message);}finally{if(alive.current)setBusy(false);}}}>读取最新草稿</button></div>
  </Disclosure>;
}
