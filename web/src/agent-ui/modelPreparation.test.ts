...
describe("task-scoped model preparation", () => {
  it("reduces preparation to one blocker without treating unknown as failed", async () => {
    const ready = await fixture().service.inspect(context, new AbortController().signal);
