...
it("creates only explicit human editable geometry without model evidence or confidence",()=>{
  for(const kind of ["classification","semantic_mask","instance_mask","bounding_box","polygon","polyline","keypoints"]){
  ...
  for(const kind of ["relation","unsupported"]){expect(()=>newHumanAnnotation("i",{id:"t",display_name:"T",kind,labels:["a"],required:false},"id","now")).toThrow();}
});
