...
describe("TaskUsage", () => {
  it("shows preset mode as no request without fabricated token or cost", () => {
    const html = renderToStaticMarkup(<TaskUsageView value={page({
      ...
      summary: { attempt_count: 0, known_cost: null, currency: null, input_tokens: 0, cached_input_tokens: 0, output_tokens: 0, unknown_attempt_count: 0 },
  ...
  it("renders immutable per-attempt model, provider, tokens, source, price revision and decimal cost", () => {
    const html = renderToStaticMarkup(<TaskUsageView value={page()} />);
    for (const text of ["Qwen Vision", "Remote Provider", "1,500", "500", "actual", "Model Profile r7", "USD 0.007", "价格快照"])
  ...
  it("keeps unknown cost and probes distinct from ordinary task attempts", () => {
    const html = renderToStaticMarkup(<TaskUsageView value={page({
      state: "partial",
      summary: { attempt_count: 2, known_cost: null, currency: null, input_tokens: null, cached_input_tokens: null, output_tokens: null, unknown_attempt_count: 1 },
  ...
  it("lists mixed currencies separately instead of adding them", () => {
    const html = renderToStaticMarkup(<TaskUsageView value={page({
      summary: { attempt_count: 2, known_cost: null, currency: null, currency_totals: [{ currency: "USD", cost: "0.007" }, { currency: "CNY", cost: "0.050" }], input_tokens: 20, cached_input_tokens: 0, output_tokens: 5, unknown_attempt_count: 0 },
