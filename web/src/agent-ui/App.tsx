...
import { TaskLifecycleMenu } from "./TaskLifecycleMenu";
...
export function AgentPreviewApp({
  ...
  const taskLifecycleChanged=async(changedTask?:string)=>{
    await adapter.refresh?.();
    if(!changedTask||task?.id!==changedTask)return;
    const latest=adapter.snapshot();
    const next=latest.tasks.find(candidate=>candidate.project===task.project&&!candidate.id.startsWith("new:"))
      ||latest.tasks.find(candidate=>candidate.project===task.project&&candidate.id.startsWith("new:"));
    if(next)navigate({settings:null,task:next.id,pane:null},true);
    else {
      const target=taskLocation(urlRef.current,task.project);target.search="";
      history.pushState(null,"",target);setUrl(target);
    }
  };
  const send = async () => {
  ...
  const managementTitle = managementPage ? ({data:"图片数据",labels:"标签定义",pipelines:"自动化方案",runs:"处理记录",batches:"批量处理",review:"审核标注",export:"导出",trash:"回收站",tasks:"Agent 任务历史"})[managementPage.page] : undefined;
  const active =
                  ...
                  state.tasks
                    ...
                    .map((t) => (
                      <div className={`task-tree-row ${!section && t.id === task?.id ? "selected" : ""}`} key={t.id}>
                        <button
                          className="task-link"
                          aria-current={!section && t.id === task?.id ? "page" : undefined}
                          onClick={() => navigate({ settings: null, task: t.id, pane: null })}
                        >
                          <span className="task-dot" aria-hidden="true" /><SidebarTitle>{t.title}</SidebarTitle>
                        </button>
                        {!fixture&&adapter.taskLifecycle&&<TaskLifecycleMenu task={t} service={adapter.taskLifecycle} onChanged={async()=>taskLifecycleChanged(t.id)}/>}
                      </div>
                    ))}
              </div>
            ))}
                  ...
                  <div className="thread">
                    {!task.items.length ? (
                      <div className="welcome">
                        ...
                        {!fixture && adapter.demoOnboarding && state.workspaceId && (
                          <DemoOnboarding service={adapter.demoOnboarding} workspaceId={state.workspaceId} onStarted={openDemoReceipt}/>
                        )}
                      </div>
                    ) : (
                      <>
