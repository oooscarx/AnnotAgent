...
it("keeps the newest failed journey visible when server history is newest-first",async()=>{
  const {transport}=mockTransport({[`${root}/t1/workspace`]:{project_id:project.project_id,project_owner_id:project.project_owner_id,conversation_id:project.conversation_id,task:{input:{id:"t1",schema_revision:"schema-1"}},agent_model:{revision:0,model_profile_id:null},actions:{},queue:[],calls:[],journey_consents:[{record:{consent:{id:"new"}},dispatch:{status:"settled",updated_at:"2026-09-10"},builder:{evidence:{outcome:"failed"}}},{record:{consent:{id:"old"}},dispatch:{status:"settled",updated_at:"2026-09-09"},sample:{status:"succeeded"}}]}});
  const adapter=new HttpAdapter(transport);await adapter.refresh();await adapter.loadTask(project.project_id,"t1");
  expect(adapter.snapshot().tasks.find(t=>t.id==="t1")?.receipts?.at(-1)).toMatchObject({id:"new",status:"failed"});
});
