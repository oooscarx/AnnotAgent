...
export type TrashService = LifecycleService & Pick<typeof api,"trash">;
...
export function TrashManagement({projectId,workspaceId,service}:{projectId:string;workspaceId:string;service:TrashService}) {
  const key=`annotagent.lifecycle.${workspaceId}.${projectId}`;
  ...
  useEffect(()=>{
    ...
    void service.trash(projectId).then(r=>{if(!current)return;if(r.items.some(item=>item.project_id!==projectId))throw new Error("回收站对象归属不匹配");setItems(r.items);}).catch(e=>{if(current)setError(e.message);});
    return()=>{current=false;};
  },[service,projectId,generation]);
  useEffect(()=>{
    pendingRef.current=undefined;setPending(undefined);setReceipt(undefined);setOperation(undefined);
    try {const raw=localStorage.getItem(key);if(!raw)return;const saved=JSON.parse(raw) as {request?:ManagementRequest;receipt?:ManagementReceipt};if(saved.request?.project_id===projectId&&saved.request.confirmation_token&&Array.isArray(saved.request.objects)){pendingRef.current=saved.request;setPending(saved.request);}if(saved.receipt?.project_id===projectId)setReceipt(saved.receipt);}
  ...
  const open=(action:"restore"|"purge",entries=targets)=>{
    ...
    setOperation({project_id:projectId,objects:entries.map(e=>e.object),action,idempotency_key:crypto.randomUUID()});
  };
  return <section className="native-project-manager"><h1>项目回收站</h1><p>恢复删除的记录，或显式清理符合条件的记录。原图、已确认标注、模型和凭证不在这里删除。</p><a href={`/projects/${encodeURIComponent(projectId)}/work`}>返回 Agent</a>
    ...
    {!items&&!error&&<p role="status">读取回收站…</p>}{items?.length===0&&<p>回收站为空。</p>}
