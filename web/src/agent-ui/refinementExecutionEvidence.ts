 ...
 * single terminal lineage contains both persisted artifacts and successful node
 * receipts for prompted segmentation and mask-to-bbox conversion.
 */
export function imageRefinementEvidence(
  ...
  const candidates = candidateIds.map((candidateId): CandidateRefinementEvidence => {
    ...
    const stages = lineageId
      ? (sample.projection?.debug_stages ?? []).filter((stage) => stage.lineage_id === lineageId)
      : [];
    const mask = stages.find((stage) => stage.stage === "mask");
    const refined = stages.find((stage) => stage.stage === "refined" && maskToBbox(stage));
    ...
    let reason = "提示分割调用、Mask Artifact 与 mask_to_bbox Artifact 已在同一终端 lineage 中核实。";
    if (!executed) {
      ...
      else reason = stoppedReason(stages, configuredRefiner);
    }
    return {
