...
type Input={request_id:string;decision:Decision};
...
export function CreateTaskSchema({service,project,conversation,task,workspace,onSaved}:{service:Pick<typeof api,"saveHumanConversationSchema">;project:string;conversation:string;task:string;workspace:string;onSaved:(draft:ConversationSchemaDraft)=>void}){
  const key=`annotagent.manual-schema.${workspace}.${project}.${conversation}.${task}`;
  ...
  useEffect(()=>{alive.current=true;try{const raw=localStorage.getItem(key);if(raw){const value=readManualSchemaCommand(raw);setPending(value);setKind(value.decision.kind);setLabels(value.decision.labels.join("\n"));setRules(value.decision.boundary_rules.join("\n"));}}catch(e){setInvalid(true);setError((e as Error).message);}return()=>{alive.current=false;};},[key]);
  ...
  const save=async()=>{
    ...
    try{const input=pending||{request_id:crypto.randomUUID(),decision:{decision:"draft" as const,kind,labels:labels.split("\n").map(s=>s.trim()).filter(Boolean),multi_label:false as const,attributes:{},boundary_rules:rules.split("\n").map(s=>s.trim()).filter(Boolean),rationale:"Human-authored annotation semantics"}};
      readManualSchemaCommand(JSON.stringify(input));localStorage.setItem(key,JSON.stringify(input));setPending(input);const saved=await service.saveHumanConversationSchema(project,conversation,task,input);
  ...
  return <Disclosure title="无需模型，手工定义标签"><section aria-label="手工定义语义草稿"><p>只创建当前任务的语义草稿，不回答已有澄清问题、不自动续跑。后续方案和样本仍需单独授权。</p>
...
<label>新草稿标签（每行一个）<textarea aria-label="新草稿标签（每行一个）" value={labels} disabled={busy||!!pending||invalid} onChange={e=>setLabels(e.target.value)}/></label><label>新草稿边界规则（可选）<textarea aria-label="新草稿边界规则（可选）" value={rules} disabled={busy||!!pending||invalid} onChange={e=>setRules(e.target.value)}/></label>
    ...
    <div className="actions"><button disabled={busy||!!pending||invalid||!dirty} onClick={()=>{setLabels("");setRules("");setKind("bounding_box");setError("");}}>取消手工输入</button><button disabled={busy||invalid||(!pending&&!labels.trim())} onClick={()=>void save()}>{pending?"核实原创建请求":"保存手工草稿"}</button></div>
  </section></Disclosure>;
}
