            ...
            boxes.map((b) => (
              ...
              >
                ...
                <g className="bbox-label" pointerEvents="none" transform={`translate(${Math.max(2/screenScale,Math.min(b.x,width-Math.min(180,width*screenScale-4)/screenScale))} ${Math.max(16/screenScale,b.y-5/screenScale)}) scale(${1/screenScale})`}>
                  <text fontSize="12" paintOrder="stroke" stroke="var(--aa-surface)" strokeWidth="1.5" strokeLinejoin="round" fill={labelColor(b.label)}><title>{task.labelNames?.[b.label]||b.label}</title>{Array.from(task.labelNames?.[b.label]||b.label).slice(0,20).join("")}{(task.labelNames?.[b.label]||b.label).length>20?"…":""}{fixture ? " · 演示" : ""}</text>
                </g>
                {selected === b.id && (
                  <circle
                    cx={b.x + b.w}
      ...
      <Disclosure className="annotation-list" title={`标注列表与精确编辑 · ${boxes.length} 个`}>
        {boxes.map((b) => (
          <div key={b.id}>
            ...
            >
              {task.labelNames?.[b.label]||b.label}
            </button>
            <label>
              标签
              {task.labelNames&&Object.keys(task.labelNames).length?<select value={b.label} disabled={!editable} onChange={e=>update(b.id,{label:e.target.value})}>{Object.entries(task.labelNames).map(([id,name])=><option key={id} value={id}>{name}</option>)}</select>:<input
                value={b.label}
                disabled={!editable}
