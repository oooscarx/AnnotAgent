...
export function ConversationSchemaCard({ project, conversation, message, onDirtyChange, onSample, onAssistance, onSetup }: { project: string; conversation: string; message: string; onDirtyChange: (dirty: boolean) => void; onAssistance?:()=>void; onSample: OpenConversationSample; onSetup?:(task?:string)=>void }) {
  ...
  return <section className="conversation-schema-card" aria-label="Annotation Schema proposal">
    ...
    {!manual && !waiting && (!receipt||clarification) && <button disabled={!ready||busy||Boolean(savedConsent)} onClick={()=>void prepare(true)}>{clarification?"Answer this clarification":"Define labels myself · no LLM needed"}</button>}
    {manual && task && <ConversationHumanSchema clarification={clarification} key={task.input.id} project={project} conversation={conversation} task={task.input.id} schemaId={humanSchema} onSaved={setHumanSchema} onDirtyChange={onDirtyChange} onSample={onSample} onAssistance={onAssistance}/>}
