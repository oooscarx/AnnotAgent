...
export function ConversationSchemaCard({ project, conversation, message }: { project: string; conversation: string; message: string }) {
  ...
  useEffect(() => {
    ...
    void api.conversationTasks(project, conversation, controller.signal).then(async (tasks) => {
      ...
      setTask(current); setReceipt(calls.at(-1)); setCallId(calls.at(-1)?.id ?? ""); setReady(true);
    }).catch((error: Error) => { if (!controller.signal.aborted) setError(error.message); });
    return () => { active.current = false; controller.abort(); };
  }, [project, conversation, message]);
  useEffect(() => {
    if (!task || !callId || (receipt && receipt.status !== "reserved")) return;
  ...
  }, [project, conversation, task?.input.id, callId, receipt?.status]);
