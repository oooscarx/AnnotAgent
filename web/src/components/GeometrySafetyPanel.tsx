...
function Metrics({ label, value }: { label: string; value: PipelineGeometryMetrics }) {
  return <section className="geometry-comparison-column">
    <h4>{label}</h4>
    <dl>
      <div><dt>{t("Semantic precision")}</dt><dd>{percent(value.semantic_precision)}</dd></div>
      <div><dt>{t("Semantic recall")}</dt><dd>{percent(value.semantic_recall)}</dd></div>
      <div><dt>{t("Median IoU")}</dt><dd>{decimal(value.median_iou)}</dd></div>
      <div><dt>P10 IoU</dt><dd>{decimal(value.p10_iou)}</dd></div>
      <div><dt>{t("Median center shift")}</dt><dd>{decimal(value.median_center_shift)}</dd></div>
      <div><dt>P90 center shift</dt><dd>{decimal(value.p90_center_shift)}</dd></div>
      <div><dt>{t("Manual resize rate")}</dt><dd>{percent(value.manual_resize_rate)}</dd></div>
      <div><dt>{t("Review rate")}</dt><dd>{percent(value.review_rate)}</dd></div>
      <div><dt>{t("Cost / image")}</dt><dd>${value.cost_per_image}</dd></div>
      <div><dt>{t("Latency / image")}</dt><dd>{value.latency_per_image_ms} ms</dd></div>
      <div><dt>{t("Failures")}</dt><dd>{value.failure_count}</dd></div>
    </dl>
    {Object.keys(value.size_buckets).length > 0 && <details>
      <summary>{t("Object-size evidence")}</summary>
...
function RunChoice({
  ...
  return <label className="improvement-run-choice">
    ...
    <span><strong>{run.id.slice(0, 8)}</strong><small>{new Date(run.updated_at).toLocaleString(localeTag())} · {title(run.status)}</small></span>
  </label>;
}
...
export function ImproveAutomationPanel({
  ...
  return <section className="panel improve-automation" id="improve-automation">
    <header className="improve-automation-header">
      <div><span className="eyebrow">{t("Evidence-driven revision")}</span><h2>{t("Improve Automation")}</h2><p>Diagnose the current immutable Version, create a focused Patch Draft, and compare it on separate holdout images. AnnotAgent never publishes this change.</p></div>
    ...
    <div className="geometry-policy-strip">
      ...
      {!policies.length && <article><span>{t("Geometry policy")}</span><strong>Loading conservative defaults…</strong></article>}
    </div>
    <div className="improvement-setup-grid">
      <label>{t("Baseline Version")}<select value={workflowKey} onChange={(event) => setWorkflowKey(event.target.value)}>{published.map((workflow) => <option key={`${workflow.workflow_id}:${workflow.version}`} value={`${workflow.workflow_id}:${workflow.version}`}>{workflow.name} · v{workflow.version}</option>)}</select></label>
      <label>{t("Bounding-box task")}<select value={taskId} onChange={(event) => setTaskId(event.target.value)}>{bboxTasks.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.display_name || candidate.id}</option>)}</select></label>
      <label>{t("Target Label")}<select value={label} onChange={(event) => setLabel(event.target.value)}>{(task?.labels ?? []).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
    </div>
    {!published.length || !bboxTasks.length ? <div className="guided-callout"><strong>{t("Improvement setup is incomplete")}</strong><p>{t("A published Workflow and bounding-box task are required.")}</p></div> : <div className="improvement-evidence-grid">
      <fieldset><legend>{t("Diagnosis evidence")}</legend><p>Reviewed Runs used to decide what is wrong.</p><div className="improvement-run-list">{projectRuns.map((run) => <RunChoice key={run.id} run={run} checked={evidenceRunIds.includes(run.id)} disabled={holdoutRunIds.includes(run.id)} onChange={(checked) => changeEvidence(run.id, checked)} />)}{!projectRuns.length && <small>No terminal Runs are available.</small>}</div></fieldset>
      <fieldset><legend>{t("Independent evaluation holdout")}</legend><p>Different Runs used to test whether the Patch actually improves.</p><div className="improvement-run-list">{projectRuns.map((run) => <RunChoice key={run.id} run={run} checked={holdoutRunIds.includes(run.id)} disabled={evidenceRunIds.includes(run.id)} onChange={(checked) => changeHoldout(run.id, checked)} />)}{!projectRuns.length && <small>No terminal Runs are available.</small>}</div></fieldset>
    </div>}
    <div className="button-row">
      ...
      <select aria-label={t("Saved improvement session")} value={active?.id ?? ""} onChange={(event) => { setActiveId(event.target.value); onSelectSession(event.target.value || undefined); }}><option value="">{t("No saved session")}</option>{sessions.map((session) => <option key={session.id} value={session.id}>{title(session.diagnosis.primary_failure_class)} · {session.id.slice(0, 8)}</option>)}</select>
    </div>
    {active && <div className="improvement-result">
      <section className="improvement-diagnosis"><span className="eyebrow">{t("Diagnosis")}</span><h3>{title(active.diagnosis.primary_failure_class)}</h3><ul>{active.diagnosis.evidence_statements.map((statement) => <li key={statement}>{statement}</li>)}</ul><div className="tag-group"><span>{active.diagnosis.geometry_correction_count} geometry corrections</span><span>{active.diagnosis.semantic_target_correct_count} semantic matches</span><span>{active.diagnosis.provider_failure_count} Provider failures</span><span>{active.diagnosis.no_candidate_count} no-candidate results</span></div></section>
      <section className="improvement-diff"><span className="eyebrow">{t("Pipeline Patch")}</span><h3>{t("Review selected changes")}</h3>{diffRows(active.diff).map((change) => <label className={change.tone} key={change.id}><input type="checkbox" checked={selectedChanges.includes(change.id)} disabled={active.status === "applied_to_draft"} onChange={(event) => setSelectedChanges((current) => event.target.checked ? [...current, change.id] : current.filter((id) => id !== change.id))} /><span>{change.tone === "added" ? "+" : change.tone === "removed" ? "−" : "~"} {change.label}</span></label>)}<small>{active.validation.valid ? "Static validation passed." : `${active.validation.issues.filter((issue) => issue.blocking).length} blocking validation issue(s).`}</small>{active.setup_requirements.map((requirement) => <p className="setup-requirement" key={requirement}>{requirement}</p>)}</section>
      <section className="improvement-actions"><span className="eyebrow">{t("Human approval boundary")}</span><div className="button-row"><button disabled={Boolean(busy) || !active.evaluation_run_ids.length || !active.validation.valid} onClick={compare}>{busy === "compare" ? "Comparing…" : "Run Before / After"}</button><button className="primary" disabled={Boolean(busy) || active.status !== "awaiting_human_approval" || !selectedChanges.length} onClick={apply}>{busy === "apply" ? "Applying…" : "Apply selected changes to Draft"}</button></div><small>Publishing remains a separate action in Test &amp; Activate.</small></section>
      {active.comparison && <section className={`geometry-comparison ${active.comparison.recommendation}`}><header><div><span className="eyebrow">{t("Independent comparison")}</span><h3>{title(active.comparison.recommendation)}</h3></div><span>{title(active.comparison.evidence_sufficiency)} evidence</span></header><div><Metrics label={t("Before")} value={active.comparison.baseline} /><Metrics label={t("After")} value={active.comparison.candidate} /></div>{[...active.comparison.reasons, ...active.comparison.regressions].length > 0 && <ul>{[...active.comparison.reasons, ...active.comparison.regressions].map((reason) => <li key={reason}>{reason}</li>)}</ul>}</section>}
    </div>}
    <details className="geometry-calibration-panel">
      <summary><span><strong>{t("Geometry calibration")}</strong><small>Exact Project, model revision, node configuration, prompt and preprocessing scope</small></span><b>{calibrations.length}</b></summary>
      <div className="improvement-setup-grid"><label>{t("Geometry-producing step")}<select value={calibrationNode} onChange={(event) => setCalibrationNode(event.target.value)}><option value="">Choose a bound detection or segmentation step</option>{geometryNodes.map((node) => <option key={node.id} value={node.id}>{node.id} · {node.model_binding}</option>)}</select></label><button disabled={Boolean(busy) || !calibrationNode || !evidenceRunIds.length} onClick={calibrate}>{busy === "calibrate" ? "Evaluating…" : t("Run geometry calibration")}</button></div>
      <div className="calibration-list">{calibrations.map((view) => <article key={view.report.id}><header><strong>{title(view.effective_status)}</strong><small>{view.report.key.task_id}{view.report.key.label_id ? ` · ${view.report.key.label_id}` : ""}</small></header><dl><div><dt>{t("Samples")}</dt><dd>{view.report.sample_count}</dd></div><div><dt>{t("Small objects")}</dt><dd>{view.report.small_object_sample_count}</dd></div><div><dt>{t("Median IoU")}</dt><dd>{decimal(view.report.median_iou)}</dd></div><div><dt>P10 IoU</dt><dd>{decimal(view.report.p10_iou)}</dd></div><div><dt>{t("Manual adjustment")}</dt><dd>{percent(view.report.manual_adjustment_rate)}</dd></div></dl>{view.staleness_reasons.length > 0 && <small>Stale because: {view.staleness_reasons.map(title).join(", ")}</small>}</article>)}</div>
