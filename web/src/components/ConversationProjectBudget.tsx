...
export function ConversationProjectBudget({project,onDirtyChange}:{project:string;onDirtyChange:(value:boolean)=>void}){
  ...
  useEffect(()=>{
    const controller=new AbortController();setError("");setConfirmed(false);
    void api.projectCallLimit(project,controller.signal).then(value=>{if(!controller.signal.aborted){setSaved(value);setMaximum(String(value.maximum_calls??""));}}).catch((failure:Error)=>{if(!controller.signal.aborted)setError(failure.message);});
  ...
  return <details className="conversation-project-budget"><summary>Project call limit</summary><section className="conversation-consent" aria-label="Project call limit">
    ...
    {saved?<p role="status">{saved.reserved_calls} calls reserved · {saved.maximum_calls===null?"No Project conversation ceiling configured":`${saved.maximum_calls} cumulative maximum`} · Revision {saved.revision} · Saved snapshot; reload for current usage.</p>:<p role="status">Loading saved Project limit…</p>}
    <label>Cumulative maximum calls<input type="number" min={saved?.reserved_calls??0} step="1" value={maximum} disabled={!saved||busy||Boolean(frozen.current)} onChange={event=>{setMaximum(event.target.value);setConfirmed(false);}}/></label>
    <label><input type="checkbox" checked={confirmed} disabled={busy||Boolean(frozen.current)} onChange={event=>setConfirmed(event.target.checked)}/>I confirm this cumulative Project limit. Model, image and destination permissions still require their own consent.</label>
