...
import { ConversationExports } from "./ConversationExports";
...
import { ProjectBudgetAccess } from "./ConversationBudgetNotice";
...
export function ConversationWorkspace({ project, titleRef, pane, conversationId, imageId, draftId, sampleTestId, taskId, humanRequestId, classReviewId, referenceMessageId, processingOperationId, exportBefore, results, onNavigate, onNavigationGuardChange }: {
  ...
  const openImage = (id: string) => {
    ...
    try { onNavigate(projectWorkPath(project.id, { conversationId: conversation, imageId: id, exportBefore, draftId, sampleTestId, taskId, humanRequestId })); }
    finally { selectingImage.current = false; }
  };
  const showResults = (next?:ConversationResultsContext) => {
    sampleNavigation.current++;
    onNavigate(projectWorkPath(project.id,{conversationId:conversation,imageId,draftId,sampleTestId,taskId,humanRequestId,processingOperationId,exportBefore,results:next}));
    setMobileView("images");
  };
  ...
  const goalCard = conversation && goalMessage && <ConversationSchemaCard prepareRequested={prepareMessage===goalMessage.input.id} key={`${conversation}:${goalMessage.input.id}`} project={project.id} conversation={conversation} message={goalMessage.input.id} onDirtyChange={schemaDirtyChange} onAssistance={assistanceChanged} onSample={(draft,test,image)=>void openSample(draft,test,image)} onSetup={selectedTask=>onNavigate(conversationSettingsPath(project.id,"providers",projectWorkPath(project.id,{conversationId:conversation,taskId:selectedTask ?? taskId,imageId,draftId,sampleTestId,humanRequestId,referenceMessageId,processingOperationId,results})))} />;
  const taskProcessing = processing.filter(operation=>operation.authorization.conversation?.task_id === referenceTask?.id);
  return <ProjectBudgetAccess.Provider value={()=>setManagementRequest(value=>value+1)}><section className="conversation-workspace agent-workspace-layout" aria-label="Annotation workspace">
    <AgentProjectHeader titleRef={titleRef} managementRequest={managementRequest} canCloseManagement={()=>!budgetDirty.current || window.confirm("Discard unsaved Project limit input?")} managementContent={<ConversationProjectBudget initiallyOpen={managementRequest>0} key={project.id} project={project.id} onDirtyChange={budgetDirtyChange}/>} projectName={project.name} tasks={tasks} messages={[...messages,...messageContext]} selectedTask={referenceTask?.id ?? taskId} ready={ready && (!conversation || requestsReady)} artifactsOpen={artifactsOpen} onToggleArtifacts={toggleArtifacts} onSelect={id=>onNavigate(projectWorkPath(project.id,{conversationId:conversation,taskId:id}))} onProjects={()=>onNavigate("/projects")} onSettings={()=>onNavigate(conversationSettingsPath(project.id,"providers",navigationContext))} onManagement={()=>onNavigate(`/projects/${encodeURIComponent(project.id)}`)} />
    ...
    <div ref={root} className="conversation-split" data-artifacts-open={artifactsOpen} data-mobile-view={mobileView} style={{ "--conversation-width": `${width}%` } as CSSProperties}>
      <section className="conversation-panel" data-empty={!messages.length && !goalMessage} aria-label="Project conversation">
        <div className="conversation-history">
        {!messages.length&&!goalMessage ? <>
          <h2>What would you like to annotate?</h2>
          <p className="muted">Describe your goal before or after uploading images.</p>
        </> : <h2 className="sr-only">Task conversation</h2>}
        ...
        <ol className="conversation-messages" aria-label="Saved messages">
          {messages.map((message) => <li key={message.input.id}><p className="agent-user-message">{message.input.text}</p>{message.input.image && <button onClick={() => {
        ...
        {taskProcessing.length>0 && <section className="conversation-consent" aria-label="Saved processing tasks"><h3>Processing tasks</h3>{taskProcessing.map(operation=><article className="conversation-consent" key={operation.id}>
        ...
        {goalMessage && !messages.some(message=>message.input.id===goalMessage.input.id) && goalCard}
      ...
      <section id="conversation-artifacts" className="conversation-image-panel" aria-label="Project images">
        ...
        <ConversationImages key={selected?.image_id ?? "unselected"} images={images} selectedId={selected?.image_id} onSelect={openImage}/>
        </>}
      </section>
    </div>
  ...
  </section></ProjectBudgetAccess.Provider>;
}
