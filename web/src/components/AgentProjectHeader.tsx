...
export function AgentProjectHeader({ projectName, tasks, messages, selectedTask, ready, artifactsOpen, titleRef, onSelect, onProjects, onSettings, onManagement, onToggleArtifacts, managementContent, canCloseManagement, managementRequest=0 }: {
  managementRequest?: number;
  managementContent?: ReactNode; canCloseManagement?: () => boolean;
  ...
  useEffect(()=>{if(managementRequest>0)setPanel("management");},[managementRequest]);
  ...
  const mayClose = () => panel !== "management" || !canCloseManagement || canCloseManagement();
  const close = () => { if (!mayClose()) return; (panel === "tasks" ? taskTrigger : menuTrigger).current?.focus(); setPanel(undefined); };
  useEffect(() => {
    if (!panel) return;
    if (panel === "tasks") searchInput.current?.focus();
    const outside = (event: PointerEvent) => {
      if (!root.current?.contains(event.target as Node) && mayClose()) setPanel(undefined);
    };
  ...
  return <header ref={root} className="agent-project-header" aria-label="Project task context" onKeyDown={event => {
    ...
    <div className="agent-task-switch">
      {entries.length > 0 ? <button ref={taskTrigger} type="button" aria-label="Choose task" aria-expanded={panel === "tasks"} aria-controls="agent-task-popover" onClick={() => { if(mayClose())setPanel(panel === "tasks" ? undefined : "tasks"); }}>
    ...
    <div className="agent-project-actions">
      ...
      <button ref={menuTrigger} type="button" aria-label="Project menu" aria-expanded={panel === "management"} aria-controls="agent-management-popover" onClick={() => { if(mayClose())setPanel(panel === "management" ? undefined : "management"); }}>⋯</button>
      {panel === "management" && <section id="agent-management-popover" className="agent-header-popover" aria-label="Project management">
        <button type="button" onClick={onManagement}>{t("Project management")}</button>
