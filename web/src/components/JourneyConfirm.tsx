...
export function JourneyConfirm({ projectId, draftId, testId, imageId, operationId, onNavigate, confirmationPath, backPath, stayOnReceipt=false, expectedConversation, viewingBatchId }: {
  ...
  return <section className="journey-scene" aria-label={t("Processing confirmation")}>
    ...
    {preview && <div className="journey-consent"><h3>{preview.goal.goal || preview.plan_name}</h3><p>{t("Uses the exact plan tested in your selected samples.")}</p>
      ...
      {preview.task_budget?.project_reserved_calls!==undefined && <p>Project conversation history: {preview.task_budget.project_reserved_calls} calls reserved across all annotation tasks, including their confirmed dataset processing. Starting another task does not clear this history. These are call counts, not a price estimate or a new authorization.</p>}
