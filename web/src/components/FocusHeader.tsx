...
import { t } from "../i18n";
import { projectBuildPath, projectJourneyPath, projectReviewPath, projectRunsPath, projectTrashPath, projectWorkPath, type WorkspaceRoute } from "../navigation";
...
export function FocusHeader({ project, route, titleRef, loaded, connection, onNavigate }: {
  ...
  const backPath = workspaceReturn ?? (revisionSetup ? projectJourneyPath(route.projectId, "revise", { draftId: route.draftId, sampleTestId: route.sampleTestId, imageId: route.imageId }) : projectId ? projectWorkPath(projectId) : parent);
