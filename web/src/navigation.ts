...
export type WorkspaceRoute =
  ...
  | { kind: "conversation"; canonicalPath: string; projectId: string; pane?: "thread" | "artifacts"; conversationId?: string; imageId?: string; draftId?: string; sampleTestId?: string; taskId?: string; humanRequestId?: string; classReviewId?: string; referenceMessageId?:string; processingOperationId?:string; exportBefore?:string; results?: ConversationResultsContext }
...
export function projectWorkPath(projectId: string, context: { pane?: "thread" | "artifacts"; conversationId?: string; imageId?: string; draftId?: string; sampleTestId?: string; taskId?:string; humanRequestId?:string; classReviewId?:string; referenceMessageId?:string; processingOperationId?:string; exportBefore?:string; results?: ConversationResultsContext } = {}): string {
  const params = new URLSearchParams();
  if (context.pane) params.set("pane", context.pane);
  if (context.conversationId) params.set("conversation", context.conversationId);
  if (context.taskId) params.set("task", context.taskId);
  if (context.exportBefore) params.set("export_before",context.exportBefore);
...
export function routeFocusKey(route: WorkspaceRoute): string {
  switch (route.kind) {
    case "conversation":
      return `conversation:${route.projectId}`;
    case "projects":
...
export function parseWorkspaceRoute(
  ...
  const conversation = clean.match(/^\/projects\/([^/]+)\/work$/);
  if (conversation) {
    const projectId = decodePathSegment(conversation[1]);
    const rawPane = params.get("pane");
    if (params.has("pane") && (params.getAll("pane").length !== 1 || (rawPane !== "thread" && rawPane !== "artifacts"))) return { kind: "notFound", invalidPath: `${clean}${search}`, canonicalPath: `${clean}${search}` };
    const pane = rawPane === "thread" || rawPane === "artifacts" ? rawPane : undefined;
    const exportBefore=params.get("export_before") || undefined;
    if(params.has("export_before") && (!exportBefore || !params.get("conversation") || !params.get("task") || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(exportBefore) || params.getAll("export_before").length!==1))return {kind:"notFound",invalidPath:`${clean}${search}`,canonicalPath:`${clean}${search}`};
    ...
    return { kind: "conversation", projectId, ...context, pane, exportBefore, results, canonicalPath: projectWorkPath(projectId, { ...context, pane, exportBefore, results }) };
  }
  ...
  if (journey) {
    ...
    const scene = journey[2] as "images" | "goal" | "samples" | "model" | "confirm" | "revise";
