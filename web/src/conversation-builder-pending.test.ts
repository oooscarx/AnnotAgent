...
import { builderMatchesClassRepair, builderMatchesHumanRepair } from "./conversation-schema-history";
...
describe("exact Builder retry recovery",()=>{
  it("recognizes a single correction's admitted operation before a working Draft exists",()=>{
    const request={id:"TEST-human",draft:"TEST-repair"};
    const entry={operation:{id:"TEST-op",task_id:"TEST-task",request_hash:"a".repeat(64),status:"reserved",evidence:{schema_id:schema.id,schema_revision:2,repair_source:{kind:"human_request",reference:{request_id:request.id,draft_id:request.draft,revision:1,content_hash:"a".repeat(64)}}}}} as ConversationBuilderItem;
    expect(builderMatchesHumanRepair(entry,schema,request)).toBe(true);
    expect(builderMatchesHumanRepair(entry,schema,{...request,id:"TEST-other"})).toBe(false);
    expect(builderMatchesHumanRepair(entry,{...schema,revision:3},request)).toBe(false);
    expect(builderMatchesClassRepair(entry,schema,request)).toBe(false);
    expect(builderMatchesHumanRepair({...entry,operation:{...entry.operation,evidence:{schema_id:schema.id,schema_revision:2}}},schema,request)).toBe(false);
  });
