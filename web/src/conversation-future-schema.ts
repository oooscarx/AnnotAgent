...
export type FutureSchemaLocal = { call_id: string; base_schema_id: string; base_schema_revision: number; scope_answer_command_id: string; context_digest: string; fields: SchemaFields; frozen?: FutureSchemaInput };
...
export function futureSchemaDefinition(base: ConversationSchemaDraft, fields: SchemaFields): SchemaDefinition {
  return { ...base.definition, goal: fields.goal.trim(), boundary_rules: lines(fields.rules), task: { ...base.definition.task, kind: fields.kind, labels: lines(fields.labels) } };
}
...
export function sameSavedFutureSchemaInput(request: FutureSchemaInput, saved: SavedFutureSchemaInput, call: string) {
  ...
  return saved.command_id === request.command_id && saved.feedback_call_id === call && saved.scope_answer_command_id === request.expected_scope_answer_command_id && saved.context_digest === request.expected_context_digest && saved.base_schema_id === request.base_schema_id && saved.base_schema_revision === request.base_schema_revision && saved.definition.goal === request.goal && task.kind === decision.kind && task.multi_label === decision.multi_label && canonical(task.labels) === canonical(decision.labels) && canonical(task.attributes) === canonical(decision.attributes) && canonical(saved.definition.boundary_rules) === canonical(decision.boundary_rules);
}
...
export function parseFutureSchemaLocal(raw: string | null, call: string): FutureSchemaLocal | undefined {
  ...
  try {
    ...
    if (value.frozen !== undefined) {
      ...
      if (Object.keys(input).some(key => !["command_id", "expected_scope_answer_command_id", "expected_context_digest", "base_schema_id", "base_schema_revision", "goal", "decision"].includes(key))) return undefined;
    }
