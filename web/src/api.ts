...
export const api = {
  ...
  conversationBuilderHistory: (project: string, conversation: string, task: string, signal?: AbortSignal, scope?: {operation_id:string;image_class_review_id?:never}|{image_class_review_id:string;operation_id?:never}) => request<{items: import("./types").ConversationBuilderItem[]}>(`/api/projects/${encodeURIComponent(project)}/conversations/${encodeURIComponent(conversation)}/tasks/${encodeURIComponent(task)}/builder-operations${scope ? `?${new URLSearchParams(Object.entries(scope).filter(([,value])=>value!==undefined).map(([key,value])=>[key,String(value)]))}` : ""}`,{signal}),
