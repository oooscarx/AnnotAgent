...
export type ProcessingAuthorization = { conversation?: {conversation_id:string;task_id:string;schema:{id:string;revision:number;definition:{goal:string;task:{labels:string[]}}}}; revision: number; authorization_fingerprint: string; image_count: number; available_images: number; maximum_model_calls: number; sample_feedback_count: number; plan_name: string; goal: { goal?: string }; models: { model_profile_id: string; remote_model_id: string; provider_base_url: string }[]; native_models?: { id: string; name: string; destination: string; revision: number }[] };
