...
export type ProcessingAuthorization = { task_budget?:{total_reserved_calls:number;total_authorized_calls:number;project_reserved_calls?:number;project_authorized_calls?:number}; conversation?: {conversation_id:string;task_id:string;schema:{id:string;revision:number;definition:{goal:string;task:{labels:string[]}}}}; revision: number; authorization_fingerprint: string; image_count: number; available_images: number; maximum_model_calls: number; sample_feedback_count: number; plan_name: string; goal: { goal?: string }; models: { model_profile_id: string; remote_model_id: string; provider_base_url: string }[]; native_models?: { id: string; name: string; destination: string; revision: number }[] };
...
export const api = {
  ...
  projectCallLimit: (project:string,signal?:AbortSignal)=>request<{revision:number;maximum_calls:number|null;reserved_calls:number}>(`/api/projects/${encodeURIComponent(project)}/conversation-call-limit`,{signal}),
  setProjectCallLimit: (project:string,input:{id:string;expected_revision:number;maximum_calls:number})=>request<{revision:number;maximum_calls:number|null;reserved_calls:number}>(`/api/projects/${encodeURIComponent(project)}/conversation-call-limit`,{method:"POST",body:JSON.stringify(input)}),
  ...
  answerHumanRequest: (project:string, value:import("./conversation-human-api").HumanRequest, answer:import("./types").SampleFeedbackRevision, journeyConsentId?:string) => request<import("./conversation-human-api").HumanRequest>(`/api/projects/${encodeURIComponent(project)}/conversations/${encodeURIComponent(value.input.conversation_id)}/tasks/${encodeURIComponent(value.input.task_id)}/human-requests/${encodeURIComponent(value.input.id)}/answer`,{method:"POST",body:JSON.stringify({answer,journey_consent_id:journeyConsentId})}),
  ...
  deferHumanRequest: (project:string,value:import("./conversation-human-api").HumanRequest,input:{command_id:string;expected_revision:number;deferred:boolean})=>request<import("./conversation-human-api").HumanRequest>(`/api/projects/${encodeURIComponent(project)}/conversations/${encodeURIComponent(value.input.conversation_id)}/tasks/${encodeURIComponent(value.input.task_id)}/human-requests/${encodeURIComponent(value.input.id)}/deferral`,{method:"POST",body:JSON.stringify(input)}),
  ...
  conversationSchemaPreview: (project: string, conversation: string, task: string, model?:string) => request<import("./types").ConversationSchemaPreview>(`/api/projects/${encodeURIComponent(project)}/conversations/${encodeURIComponent(conversation)}/tasks/${encodeURIComponent(task)}/schema-preview${model?`?model_id=${encodeURIComponent(model)}`:""}`),
  ...
  conversationBuilderHistory: (project: string, conversation: string, task: string, signal?: AbortSignal, scope?: {operation_id:string;image_class_review_id?:never;repair_request_id?:never}|{image_class_review_id:string;operation_id?:never;repair_request_id?:never}|{repair_request_id:string;operation_id?:never;image_class_review_id?:never}) => request<{items: import("./types").ConversationBuilderItem[]}>(`/api/projects/${encodeURIComponent(project)}/conversations/${encodeURIComponent(conversation)}/tasks/${encodeURIComponent(task)}/builder-operations${scope ? `?${new URLSearchParams(Object.entries(scope).filter(([,value])=>value!==undefined).map(([key,value])=>[key,String(value)]))}` : ""}`,{signal}),
  ...
  submitConversationMessage: (projectId: string, conversationId: string, input: import("./conversation-send").SendCommand) =>
    request<import("./conversation-send").SendReceipt>(`/api/projects/${encodeURIComponent(projectId)}/conversations/${encodeURIComponent(conversationId)}/send`, { method: "POST", body: JSON.stringify(input) }),
  ...
  conversationProcessing: (projectId:string, conversation:string, task:string, signal?:AbortSignal) => request<ProcessingReceipt[]>(`/api/projects/${encodeURIComponent(projectId)}/conversations/${encodeURIComponent(conversation)}/tasks/${encodeURIComponent(task)}/processing-operations`, {signal}),
