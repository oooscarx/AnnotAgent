...
function SettingsWorkspace({
  ...
  return (
    <section className="page-stack">
      ...
      {section === "providers" && (
        <a href="/settings/providers">管理 Provider（新设置）</a>
      )}
      {section === "models" && (
        <a href="/settings/agent-models">管理模型与全局默认（新设置）</a>
      )}
...
function TaskModelPreparation({ section, onSelect, onClose, onError }: {
  ...
  return <section ref={panel} tabIndex={-1} className="task-model-preparation panel" aria-label={t("Task model preparation")}>
    ...
    {section === "models" && <a href="/settings/agent-models">管理模型与全局默认（新设置）</a>}
    {section === "providers" && <a href="/settings/providers">管理 Provider（新设置）</a>}
    {section === "plugins" && <ExpertModelPluginsPage onError={onError} />}
  </section>;
}
