...
export function App() {
  ...
  useEffect(() => subscribeEvents((event) => {
    ...
    if (["run_created", "run_started", "run_paused", "run_resumed", "run_cancelled", "run_budget_exceeded", "run_completed", "review_requested", "run_failed", "run_interrupted"].includes(event.kind)) void refresh();
  }, () => { void refresh(); }), []);
  ...
  return <div className="app-shell">
    ...
    <main id="main-content">
      ...
      {page === "project" && <ProjectPage project={selectedProject} runs={runs} events={events} onRefresh={refresh} onOpenWorkflows={() => setPage("workflows")} onError={setError} />}
      {page === "workflows" && <WorkflowsPage projects={projects} activeProjectId={projectId} onActivate={setProjectId} onError={setError} />}
...
function Dashboard({ projects, runs, models, skills, reviewQueue, onSelect, onRefresh }: { projects: ProjectSummary[]; runs: HistoryRun[]; models: ModelBinding[]; skills: EnabledSkill[]; reviewQueue: number; onSelect: (id: string) => void; onRefresh: () => void }) {
  const activeRuns = runs.filter((run) => run.controllable && (run.status === "running" || run.status === "paused")).length;
...
function ProjectPage({ project, runs, events, onRefresh, onOpenWorkflows, onError }: { project?: ProjectSummary; runs: HistoryRun[]; events: RunEvent[]; onRefresh: () => void; onOpenWorkflows: () => void; onError: (value: string) => void }) {
  ...
  const visibleStatus = (latestState?.payload.data.to as string | undefined) ?? activeSummary?.status ?? (activeRun ? "running" : projectRuns[0]?.status ?? "pending");
  const start = () => { setStarting(true); void api.startRun(project.id).then(onRefresh).catch((error: Error) => onError(error.message)).finally(() => setStarting(false)); };
  const control = (action: "pause" | "resume" | "cancel") => activeRun && api.control(activeRun, action).then(onRefresh).catch((error: Error) => onError(error.message));
  return <section className="page-stack">
    <div className="toolbar-panel project-heading"><div><span className="eyebrow">Project · {project.id}</span><h2>{project.name}</h2><p>{project.description || "No Project description provided."}</p><div className="context-line"><span>Workflow: {project.active_workflow.name}@v{project.active_workflow.version}</span><span>Skills: {project.enabled_skills.map((skill) => skill.display_name).join(", ") || "None"}</span></div></div><div className="button-row" aria-label="Run controls"><button className="primary" disabled={restoredRun.startDisabled || starting} title={activeRun ? "This Project already has an active Run" : undefined} onClick={start}>{starting ? "Starting…" : "Start image run"}</button><button disabled={!activeRun || visibleStatus !== "running"} onClick={() => control("pause")}><img src="/brand/core/icons/pause.svg" alt="" aria-hidden="true" />Pause</button><button disabled={!activeRun || visibleStatus !== "paused"} onClick={() => control("resume")}><img src="/brand/core/icons/resume.svg" alt="" aria-hidden="true" />Resume</button><button className="danger" disabled={!activeRun} onClick={() => control("cancel")}><img src="/brand/core/icons/cancel.svg" alt="" aria-hidden="true" />Cancel</button></div></div>
    <div className="run-state-grid"><Panel title="Active Run" eyebrow="Server-owned state">{project.active_batch ? <><Fact label="Batch" value={project.active_batch.id.slice(0, 8)} /><Status status={project.active_batch.status} />{project.active_batch_progress && <Fact label="Images" value={`${project.active_batch_progress.completed_images}/${project.active_batch_progress.total_images}`} />}<Fact label="Progress events" value={String(project.active_batch.event_sequence)} /></> : project.active_run ? <><Fact label="Run" value={project.active_run.id.slice(0, 8)} /><Status status={project.active_run.status} /></> : <Empty title="No active Run" detail="Start is available because the backend has no Pending, Running, or Paused Run for this Project." />}</Panel><Panel title="Last Run" eyebrow="Terminal history">{project.last_run ? <><Fact label="Run" value={project.last_run.id.slice(0, 8)} /><Status status={project.last_run.status} />{project.last_run.terminal_reason && <small className="run-reason">{project.last_run.terminal_reason}</small>}</> : <Empty title="No completed Run" detail="Terminal history will appear here." />}</Panel></div>
    {activeRun && <div className="run-progress aa-dark" aria-live="polite"><div><span className="live-dot" aria-hidden="true" /><strong>Run {activeRun.slice(0, 8)}</strong><small>{lastTask ?? "restored active Run"} · {runEvents.at(-1)?.kind.replaceAll("_", " ") ?? visibleStatus.replaceAll("_", " ")}</small></div><div className="progress-track"><i style={{ width: `${Math.min(100, Math.max(6, runEvents.length * 3))}%` }} /></div><pre>{usage ? JSON.stringify(usage.payload.data, null, 2) : activeSummary ? `${(activeSummary.input_tokens + activeSummary.output_tokens).toLocaleString()} tokens\n$${activeSummary.cost}` : "usage pending"}</pre></div>}
    <div className="project-overview-grid">
      ...
      <Panel title="Versions, Runs, Reviews & Exports" eyebrow="Project outputs"><Fact label="Workflow versions" value={project.available_workflow_versions.length} /><Fact label="Runs" value={projectRuns.length} /><Fact label="Runs awaiting review" value={projectRuns.filter((run) => run.status === "completed_with_review").length} /><TagGroup title="Export formats" values={project.export_formats} /></Panel>
    </div>
...
function WorkflowsPage({ projects, activeProjectId, onActivate, onError }: { projects: ProjectSummary[]; activeProjectId: string; onActivate: (id: string) => void; onError: (value: string) => void }) {
  ...
  const [report, setReport] = useState<WorkflowValidationReport>();
  const [busy, setBusy] = useState(false);
  const refreshDrafts = () => api.workflowDrafts(activeProjectId || undefined).then((value) => { setDrafts(value.drafts); setDraft((current) => value.drafts.find((item) => item.id === current?.id) ?? value.drafts[0]); }).catch((error: Error) => onError(error.message));
  useEffect(() => { void refreshDrafts(); setReport(undefined); }, [activeProjectId]);
  const finish = (promise: Promise<unknown>) => { setBusy(true); void promise.then(refreshDrafts).catch((error: Error) => onError(error.message)).finally(() => setBusy(false)); };
  const suggest = () => activeProjectId ? finish(api.suggestWorkflow(activeProjectId)) : onError("Select a Project before suggesting a Workflow.");
  const save = () => draft && finish(api.saveWorkflowDraft(draft));
  const dryRun = () => draft && (setBusy(true), void api.dryRunWorkflow(draft.id).then(setReport).then(refreshDrafts).catch((error: Error) => onError(error.message)).finally(() => setBusy(false)));
  const publish = () => draft && finish(api.publishWorkflow(draft.id));
  const updateNode = (index: number, patch: Partial<WorkflowDraft["nodes"][number]>) => draft && setDraft({ ...draft, nodes: draft.nodes.map((node, nodeIndex) => nodeIndex === index ? { ...node, ...patch } : node) });
  const immutable = draft?.status === "published";
  return <section className="page-stack"><div className="toolbar-panel"><div><span className="eyebrow">Suggested → validated → immutable</span><h2>Workflow Designer</h2><p>Drafts can only reference registered node and model IDs. Dry Run performs static graph and capability validation without executing models.</p></div><div className="button-row"><button onClick={suggest} disabled={busy || !activeProjectId} title={!activeProjectId ? "Select a Project first" : "Create a safe registry-bound suggestion"}>Suggest with LLM</button><button onClick={save} disabled={busy || !draft || immutable} title={immutable ? "Published versions are immutable" : "Persist the current draft"}>Save Draft</button><button onClick={dryRun} disabled={busy || !draft} title={!draft ? "Select or create a draft" : "Validate graph, nodes, and model capabilities"}>Dry Run</button><button className="primary" onClick={publish} disabled={busy || !draft || immutable} title={immutable ? "This version is already immutable" : "Publishing is blocked if Dry Run finds issues"}>Publish</button></div></div><div className="workflow-layout"><aside className="panel workflow-list"><span className="eyebrow">Workflow Drafts</span><h2>{drafts.length} drafts</h2>{drafts.map((item) => <button key={item.id} className={draft?.id === item.id ? "active" : ""} onClick={() => { setDraft(item); setReport(undefined); }}><span><strong>{item.name}</strong><small>{projects.find((project) => project.id === item.project_id)?.name ?? item.project_id}</small></span><Status status={item.status} /></button>)}{drafts.length === 0 && <Empty title="No drafts" detail="Use Suggest with LLM to create a safe registry-bound draft." />}<span className="eyebrow workflow-published-title">Published compatibility workflows</span>{entries.map(({ project, workflow }) => <button key={`${project.id}-${workflow.workflow_id}-${workflow.version}`} onClick={() => onActivate(project.id)}><span><strong>{workflow.name}</strong><small>{project.name} · v{workflow.version}</small></span><Status status={workflow.status} /></button>)}</aside><div>{draft ? <Panel title={draft.name} eyebrow={`${draft.status} · ${draft.id.slice(0, 8)}`}><div className="workflow-nodes editable-workflow">{draft.nodes.map((node, index) => <article key={node.id}><span className="node-index">{String(index + 1).padStart(2, "0")}</span><div><label>Node ID<input value={node.id} disabled={immutable} onChange={(event) => updateNode(index, { id: event.target.value })} /></label><div className="form-grid"><label>Node type<select value={node.node_type} disabled={immutable} onChange={(event) => updateNode(index, { node_type: event.target.value })}>{["vision_language", "object_detection", "semantic_segmentation", "instance_segmentation", "prompted_segmentation", "classification", "keypoint_detection", "static_validator", "review_gate", "commit"].map((value) => <option key={value}>{value}</option>)}</select></label><label>Model binding<input value={node.model_binding ?? ""} disabled={immutable} placeholder="default-vision" onChange={(event) => updateNode(index, { model_binding: event.target.value || undefined })} /></label><label>Fallback node<input value={node.fallback ?? ""} disabled={immutable} placeholder="none" onChange={(event) => updateNode(index, { fallback: event.target.value || undefined })} /></label><label>Retries<input type="number" min="0" value={node.max_retries} disabled={immutable} onChange={(event) => updateNode(index, { max_retries: Number(event.target.value) })} /></label></div><label>Depends on<input value={node.depends_on.join(", ")} disabled={immutable} onChange={(event) => updateNode(index, { depends_on: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) })} /></label><div className="form-grid"><label>Validators<input value={node.validators.join(", ")} disabled={immutable} onChange={(event) => updateNode(index, { validators: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) })} /></label><label>Refiners<input value={node.refiners.join(", ")} disabled={immutable} onChange={(event) => updateNode(index, { refiners: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) })} /></label></div><label className="checkbox-line"><input type="checkbox" checked={node.review_gate} disabled={immutable} onChange={(event) => updateNode(index, { review_gate: event.target.checked })} />Human review gate</label></div></article>)}</div>{report && <div className={report.valid ? "validation-report valid" : "validation-report invalid"}><strong>{report.valid ? "Dry Run passed" : `${report.issues.length} validation issues`}</strong>{report.execution_order.length > 0 && <small>Order: {report.execution_order.join(" → ")}</small>}{report.issues.map((issue) => <p key={`${issue.path}-${issue.code}`}><code>{issue.code}</code> {issue.path}: {issue.message}</p>)}</div>}</Panel> : selected ? <WorkflowDetail project={selected.project} workflow={selected.workflow} /> : <Empty title="Select a Workflow" detail="Choose a Project and create or select a Draft." />}</div></div></section>;
}
...
function Trace({ events }: { events: RunEvent[] }) {
  const icon = (kind: string) => kind.includes("model") ? "model-call" : kind.includes("tool") || kind.includes("artifact") ? "tool-call" : kind.includes("validation") ? "validate" : kind.includes("review") ? "review" : "agent-trace";
  const summary = (event: RunEvent) => typeof event.payload.data.summary === "string" ? event.payload.data.summary : undefined;
  return <div className="trace-panel panel"><div><span className="eyebrow">Visible execution events</span><h3>Agent trace</h3><small>Original candidates, refined Artifacts, validation, and final commits · no hidden chain-of-thought</small></div><div className="trace-strip" aria-label="Agent trace events">{events.slice(-10).map((event) => <article key={event.event_id}><span><img src={`/brand/core/icons/${icon(event.kind)}.svg`} alt="" aria-hidden="true" /></span><div><strong>{event.kind.replaceAll("_", " ")}</strong><small>{summary(event) ?? `${event.task_id ?? "run"} · ${new Date(event.occurred_at).toLocaleTimeString()}`}</small></div></article>)}</div></div>;
}
...
function Status({ status }: { status: string }) { const normalized = status.replaceAll(" ", "_").toLowerCase(); const presentation = normalized === "completed" || normalized === "confirmed" || normalized === "auto_accepted" || normalized === "published" || normalized === "valid" ? { tone: "auto-accepted", label: normalized === "published" ? "Published" : normalized === "valid" ? "Valid" : "Completed" } : normalized === "completed_with_review" || normalized === "needs_review" ? { tone: "needs-review", label: "Completed with review" } : normalized === "partial" ? { tone: "needs-review", label: "Partial" } : normalized === "cancelled" || normalized === "rejected" || normalized === "archived" ? { tone: "rejected", label: normalized === "archived" ? "Archived" : "Cancelled" } : normalized === "failed" || normalized === "budget_exceeded" || normalized === "interrupted" ? { tone: "failed", label: normalized === "interrupted" ? "Interrupted" : normalized === "budget_exceeded" ? "Budget exceeded" : "Failed" } : normalized === "running" || normalized === "paused" ? { tone: "running", label: normalized === "paused" ? "Paused" : "Running" } : { tone: "draft", label: normalized === "pending" ? "Pending" : "Draft" }; return <span className={`status status-${presentation.tone}`}>{presentation.label}</span>; }
