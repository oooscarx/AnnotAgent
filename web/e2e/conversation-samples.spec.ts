...
test(`conversation ${scenario} authorizes HTTP fixture samples and restores editable terminal canvas`,async({page,request})=>{
  ...
  if(humanSchema){
    ...
    await expect(page.getByText("No outstanding visual requests for this goal.",{exact:true})).toBeVisible();
  ...
  if(scenario==="bbox")await page.screenshot({path:isolatedEvidencePath(evidencePath("automatic-label-draft.png")),fullPage:true,animations:"disabled"});
  }
  if(scenario.startsWith("joint-ui")){
    ...
    await consent.screenshot({path:isolatedEvidencePath(evidencePath("joint-consent.png")),animations:"disabled"});
    ...
    await page.screenshot({path:isolatedEvidencePath(evidencePath("joint-result.png")),fullPage:true,animations:"disabled"});
    ...
    await expect(page.getByLabel("Saved sample results",{exact:true})).toBeVisible();
    await page.screenshot({path:isolatedEvidencePath(evidencePath("joint-result-390.png")),fullPage:true,animations:"disabled"});
    return;
  }
  ...
  if(scenario==="human-classification"){
    ...
    await authorization.getByLabel("Project budget before inference",{exact:true}).screenshot({path:isolatedEvidencePath(evidencePath("sample-project-budget-exhausted.png")),animations:"disabled"});
  ...
  if(scenario==="bbox"){
    ...
    await page.getByLabel("Sample request outcome unknown",{exact:true}).screenshot({path:isolatedEvidencePath(evidencePath("sample-outcome-unknown.png")),animations:"disabled"});
  ...
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`sample-${scenario}.png`)),fullPage:true});
  await page.setViewportSize({width:390,height:844});
  await page.getByRole("button",{name:/Images \(/}).click();
  await expect(page.getByLabel("Saved sample results",{exact:true})).toBeVisible();
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`sample-${kind}-390.png`)),fullPage:true});
  ...
  if(scenario==="bbox"){
    ...
    await page.screenshot({path:isolatedEvidencePath(evidencePath("request-task-history.png")),fullPage:true});
  ...
  if(scenario==="bbox"){
    ...
    await page.screenshot({path:isolatedEvidencePath(evidencePath("deferred-request.png")),fullPage:true});
  ...
  await page.locator(".conversation-image-panel").evaluate(element=>element.scrollTop=0);
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`human-request-${kind}.png`)),fullPage:true});
  await page.setViewportSize({width:390,height:844});
  await page.getByRole("button",{name:/Images \(/}).click();
  await expect(page.getByRole("button",{name:"Submit correction",exact:true})).toBeVisible();
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`human-request-${kind}-390.png`)),fullPage:true});
  await page.setViewportSize({width:1280,height:800});
  let answer:any;
  ...
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`repair-${kind}.png`)),fullPage:true});
  ...
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`comparison-origin-${kind}.png`)),fullPage:true});
  expect(await (await request.get(`${taskRoot}/calls`)).json()).toEqual(afterComparison);
  if(kind==="bbox"){
  ...
  await page.screenshot({path:isolatedEvidencePath(evidencePath("automatic-human-request.png")),fullPage:true});
  await page.getByRole("button",{name:"Submit correction",exact:true}).click();
  ...
  if(kind==="bbox"){
    ...
    await page.screenshot({path:isolatedEvidencePath(evidencePath("processing-confirm-390.png")),fullPage:true,animations:"disabled"});
    await page.setViewportSize({width:1280,height:800});
  }
  await confirmCard.evaluate(element=>element.scrollIntoView({block:"start"}));
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`processing-confirm-${scenario}.png`)),fullPage:true,animations:"disabled"});
  ...
  if(scenario==="bbox") await liveStatus.screenshot({path:isolatedEvidencePath(evidencePath("processing-current-status.png")),animations:"disabled"});
  ...
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`processing-linked-${scenario}.png`)),fullPage:true});
  ...
  if(requiresReview) {
    ...
    if(requiresReview) {
      ...
      await page.screenshot({path:isolatedEvidencePath(evidencePath(`formal-export-${kind}.png`)),fullPage:true,animations:"disabled"});
      await page.goBack();
  ...
  if(requiresReview) {
    ...
    await expect(exports.getByRole("link",{name:"Download annotation archive"})).toBeVisible();
    await exports.screenshot({path:isolatedEvidencePath(evidencePath(`export-card-${kind}.png`)),animations:"disabled"});
    if(scenario==="classification-review"){
      ...
      await page.reload();
      await expect(exports).toContainText("TEST pagination 21");
      await exports.screenshot({path:isolatedEvidencePath(evidencePath("export-earlier-page-restored-TEST.png")),animations:"disabled"});
  ...
  await page.screenshot({path:isolatedEvidencePath(evidencePath(`processing-results-${scenario}.png`)),fullPage:true,animations:"disabled"});
  ...
  if(humanSchema){
    ...
    await page.screenshot({path:isolatedEvidencePath(evidencePath(`candidate-message-${kind}-390.png`)),fullPage:true,animations:"disabled"});
    await reference.screenshot({path:isolatedEvidencePath(evidencePath(`candidate-reference-chip-${kind}.png`)),animations:"disabled"});
    await page.setViewportSize({width:1280,height:800});
    ...
    await historical.screenshot({path:isolatedEvidencePath(evidencePath(`message-reference-reopen-${kind}.png`)),animations:"disabled"});
