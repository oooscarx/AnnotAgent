...
test("native context file validation refuses invalid and oversized files without mutation",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const m=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8"));const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});await page.goto(`/projects/${m.project}/manage/tasks`);const area=page.getByRole("region",{name:"会话 JSON 归档",exact:true});const file=area.getByLabel("选择会话归档",{exact:true});await file.setInputFiles({name:"invalid.json",mimeType:"application/json",buffer:Buffer.from('{"format":"not-a-context"}')});await expect(area.getByRole("alert")).toContainText("不是支持的");await file.setInputFiles({name:"large.json",mimeType:"application/json",buffer:Buffer.alloc(1048577,32)});await expect(area.getByRole("alert")).toContainText("超过 1 MiB");await expect(area.getByRole("button",{name:"确认加载 · 不运行任务",exact:true})).toHaveCount(0);expect(writes).toEqual([]);
});
test("native context archive download preview import and reload preserve inert evidence",async({page,request})=>{
  ...
  await page.goto(`/projects/${b.project}/manage/tasks?task=${b.task_id}`);const area=page.getByRole("region",{name:"会话 JSON 归档",exact:true});const downloadPromise=page.waitForEvent("download");await area.getByRole("button",{name:/保存会话 JSON/}).click();const download=await downloadPromise;const path=await download.path();expect(path).toBeTruthy();const archive=JSON.parse(readFileSync(path!,"utf8"));expect(archive.format).toBe("annotagent.context");expect(archive.payload.integrity.truncated).toBe(false);
...
test("native context import recovers lost response without dispatching or repeating POST",async({page,request})=>{
  ...
  let imports=0;await page.route(`**${root}/context-imports`,async route=>{imports++;const response=await route.fetch();expect(response.ok(),await response.text()).toBeTruthy();await route.abort("failed");});const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});await page.goto(`/projects/${b.project}/manage/tasks`);await page.getByLabel("选择会话归档",{exact:true}).setInputFiles({name:"TEST-lost-response.json",mimeType:"application/json",buffer:Buffer.from(JSON.stringify(archive))});await page.getByRole("button",{name:"确认加载 · 不运行任务",exact:true}).click();await expect(page.getByRole("region",{name:"已加载会话上下文",exact:true})).toBeVisible();expect(imports).toBe(1);expect(page.url()).toContain("import_command=");await page.reload();await expect(page.getByRole("region",{name:"已加载会话上下文",exact:true})).toBeVisible();expect(imports).toBe(1);expect(writes.every(v=>v.includes("/context-imports"))).toBe(true);
});
