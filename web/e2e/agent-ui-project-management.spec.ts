...
const fixtureManifest = () => {
  const value = JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!, "utf8"));
  expect(value.fixture).toBe("external-model-only");
  expect(value.project).toMatch(/^TEST-/);
  return value;
};
test("native feedback scope recovers a real saved choice without changing sample results",async({page,request})=>{
  const stopCase=process.env.AGENT_UI_FUTURE_STOP==="1";
...
const open=async()=>{await page.locator("summary").filter({hasText:"反馈记录与修改范围"}).click();await page.getByRole("region",{name:"任务反馈记录",exact:true}).locator("summary").filter({hasText:message.text}).click();return page.getByRole("region",{name:"任务反馈记录",exact:true}).locator("details").filter({has:page.locator("summary").filter({hasText:message.text})});};await page.goto(`/projects/${m.project}/work?task=${m.task_id}`);let scope=await open();await scope.getByRole("radio",{name:"后续任务的标签规则",exact:true}).check();await scope.getByRole("button",{name:"保存反馈范围",exact:true}).click();await expect(scope.getByRole("alert")).toBeVisible();await page.reload();scope=await open();await expect(scope).toContainText("已保存范围：后续任务的标签规则");expect(sent).toHaveLength(1);expect(writes).toEqual([`${path}/scope-answer`]);expect(sent[0].expected_context_digest).toBe(status.scope_context_digest);expect((await(await request.get(path)).json()).scope_answer.input).toEqual(sent[0]);expect((await(await request.get(`/api/workflow-drafts/${record.draft_id}/sample-test?test_id=${record.id}`)).json()).sample_test).toEqual(record);
  ...
  const made=await request.post("/api/model-profiles",{headers,data:{provider_id:m.provider_id,display_name:"TEST native future proposer",remote_model_id:stopCase?"e2e-future-schema-proposal-manual-slow":"e2e-future-schema-proposal-valid",input_modalities:["text"],task_capabilities:["text_generation"],protocol_features:{tool_calls:true,structured_output:true}}});expect(made.ok(),await made.text()).toBeTruthy();const proposer=await made.json();const permission=await request.post("/api/session/privileged-confirmation",{headers,data:{action:`POST /api/providers/${m.provider_id}/active-probe`,confirmed:true}});expect(permission.ok()).toBeTruthy();const probed=await request.post(`/api/providers/${m.provider_id}/active-probe`,{headers:{...headers,"x-annotagent-privileged-confirmation":(await permission.json()).confirmation_token},data:{model_profile_id:proposer.id,confirmed_billable:true}});expect(probed.ok(),await probed.text()).toBeTruthy();await page.route(`**${path}/future-schema/proposal-preview?*`,async route=>{const url=new URL(route.request().url());url.searchParams.set("model_id",proposer.id);await route.continue({url:url.href});});
  await scope.locator("summary").filter({hasText:"后续任务规则草稿"}).click();let rules=scope.getByRole("region",{name:"后续任务规则编辑",exact:true});await rules.locator("summary").filter({hasText:"模型辅助规则建议"}).click();const controls=rules.getByRole("region",{name:"生成模型规则建议",exact:true});await controls.getByRole("button",{name:"查看生成建议的授权范围…",exact:true}).click();const proposalDialog=page.getByRole("dialog",{name:"确认模型规则建议范围",exact:true});await expect(proposalDialog).toContainText("127.0.0.1");await expect(proposalDialog.getByRole("button",{name:"生成规则建议",exact:true})).toBeDisabled();await proposalDialog.getByRole("checkbox").check();let droppedAuthorization=false;await page.route(`**${path}/future-schema/proposal`,async route=>{if(droppedAuthorization||route.request().method()!=="POST"){await route.continue();return;}const response=await route.fetch();expect(response.ok(),await response.text()).toBeTruthy();droppedAuthorization=true;await route.abort();});await proposalDialog.getByRole("button",{name:"生成规则建议",exact:true}).click();await expect(controls.getByRole("alert")).toBeVisible();const originalAuthorization=(await(await request.get(`${path}/future-schema/proposal`)).json()).authorization.consent;expect(writes.filter(p=>p===`${path}/future-schema/proposal/execute`)).toHaveLength(0);await page.evaluate(()=>{for(const key of Object.keys(localStorage))if(key.startsWith("annotagent.native-future-rules.")&&key.endsWith(".proposal-execution"))localStorage.removeItem(key);});await page.reload();scope=await open();await scope.locator("summary").filter({hasText:"后续任务规则草稿"}).click();rules=scope.getByRole("region",{name:"后续任务规则编辑",exact:true});await rules.locator("summary").filter({hasText:"模型辅助规则建议"}).click();await controls.getByRole("button",{name:"查看原授权并继续…",exact:true}).click();await proposalDialog.getByRole("checkbox").check();await proposalDialog.getByRole("button",{name:"生成规则建议",exact:true}).click();if(stopCase){await expect.poll(async()=>(await(await request.get(`${path}/future-schema/proposal`)).json()).receipt?.status).toBe("reserved");const baseState=await(await request.get(`${path}/future-schema`)).json();const baseTask=baseState.base_schema.definition.task;const fork=await request.post(`${path}/future-schema`,{headers,data:{command_id:randomUUID(),expected_scope_answer_command_id:baseState.source.scope_answer_command_id,expected_context_digest:baseState.source.context_digest,base_schema_id:baseState.base_schema.id,base_schema_revision:baseState.base_schema.revision,goal:"TEST independently saved while model running",decision:{decision:"draft",kind:baseTask.kind,labels:baseTask.labels,multi_label:baseTask.multi_label,attributes:baseTask.attributes,boundary_rules:["TEST independent"],rationale:"TEST other tab"}}});expect(fork.ok(),await fork.text()).toBeTruthy();const forked=await fork.json();await page.reload();scope=await open();await scope.locator("summary").filter({hasText:"后续任务规则草稿"}).click();rules=scope.getByRole("region",{name:"后续任务规则编辑",exact:true});await expect(rules).toContainText("未来规则草稿已保存");await rules.locator("summary").filter({hasText:"模型辅助规则建议"}).click();await expect(controls.getByRole("button",{name:"停止规则建议",exact:true})).toBeEnabled();await controls.getByRole("button",{name:"停止规则建议",exact:true}).click();await expect.poll(async()=>(await(await request.get(`${path}/future-schema/proposal`)).json()).cancelled).toBe(true);await page.reload();scope=await open();await scope.locator("summary").filter({hasText:"后续任务规则草稿"}).click();await scope.locator("summary").filter({hasText:"模型辅助规则建议"}).click();await expect(scope).toContainText("cancelled");expect(await(await request.get(`${path}/future-schema`)).json()).toEqual(forked);expect(writes.filter(p=>p===`${path}/future-schema/proposal/execute`)).toHaveLength(1);expect((await(await request.get(`/api/workflow-drafts/${record.draft_id}/sample-test?test_id=${record.id}`)).json()).sample_test).toEqual(record);return;}await expect(rules).toContainText("建议可供人工检查");expect((await(await request.get(`${path}/future-schema/proposal`)).json()).authorization.consent).toEqual(originalAuthorization);const proposalStatus=await(await request.get(`${path}/future-schema/proposal`)).json();expect(proposalStatus.proposal.Ok.decision.decision).toBe("draft");await expect(rules).toContainText(proposalStatus.proposal.Ok.decision.rationale);page.once("dialog",d=>d.accept());await rules.getByRole("button",{name:"采用到规则编辑区",exact:true}).click();await expect(rules.getByLabel("未来规则标签",{exact:true})).toHaveValue(proposalStatus.proposal.Ok.decision.labels.join("\n"));await rules.getByLabel("未来规则边界",{exact:true}).fill("TEST use visible boundaries only");await expect(rules.getByRole("region",{name:"与已测试规则的差异",exact:true})).toContainText("TEST use visible boundaries only");await rules.getByRole("button",{name:"保存未来规则草稿",exact:true}).click();await expect(rules.getByRole("alert")).toBeVisible();const savedRules=await(await request.get(`${path}/future-schema`)).json();expect(savedRules.record).toBeTruthy();
  ...
  await rules.getByRole("button",{name:"使用此规则构建新方案",exact:true}).click();const futureBuilder=rules.getByRole("region",{name:"从语义草稿构建方案",exact:true});const previewRequest=page.waitForRequest(r=>r.url().includes("/builder-preview?"));await futureBuilder.getByRole("button",{name:"检查此草稿的构建授权…",exact:true}).click();const selected=new URL((await previewRequest).url());expect(selected.searchParams.get("schema_id")).toBe(savedRules.schema.id);expect(selected.searchParams.get("schema_revision")).toBe(String(savedRules.schema.revision));const dialog=page.getByRole("dialog",{name:"确认语义草稿构建范围",exact:true});await expect(dialog).toContainText("费用未知");await expect(dialog.getByRole("button",{name:"生成方案草稿",exact:true})).toBeDisabled();await dialog.getByRole("button",{name:"取消",exact:true}).click();expect(writes).toEqual([`${path}/scope-answer`,`${path}/future-schema/proposal`,`${path}/future-schema/proposal/execute`,`${path}/future-schema`]);
  await futureBuilder.getByRole("button",{name:"检查此草稿的构建授权…",exact:true}).click();await expect(dialog).toContainText("127.0.0.1");await dialog.getByRole("checkbox").check();const launchRequest=page.waitForRequest(r=>new URL(r.url()).pathname===`${tr}/builder-operations`&&r.method()==="POST");await dialog.getByRole("button",{name:"生成方案草稿",exact:true}).click();const builderInput=(await launchRequest).postDataJSON();const operation=builderInput.selection.operation_id;expect(builderInput.selection.schema_id).toBe(savedRules.schema.id);await expect.poll(async()=>{const result=await(await request.get(`${tr}/builder-operations?operation_id=${operation}`)).json();return result.items[0]?.operation.status;},{timeout:45000}).toBe("completed");const built=(await(await request.get(`${tr}/builder-operations?operation_id=${operation}`)).json()).items[0];expect(built.operation.evidence.draft_id).toBeTruthy();expect(built.operation.evidence.draft_id).not.toBe(record.draft_id);expect(built.operation.evidence.schema_id).toBe(savedRules.schema.id);expect(built.operation.evidence.schema_revision).toBe(savedRules.schema.revision);await futureBuilder.locator("summary").filter({hasText:"构建记录 · completed"}).click();await expect(futureBuilder.getByRole("link",{name:"查看实际方案草稿",exact:true})).toHaveAttribute("href",`/projects/${m.project}/manage/pipelines/${built.operation.evidence.draft_id}`);expect((await(await request.get(`/api/workflow-drafts/${record.draft_id}/sample-test?test_id=${record.id}`)).json()).sample_test).toEqual(record);
...
test("native context file validation refuses invalid and oversized files without mutation",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const m=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8"));const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});await page.goto(`/projects/${m.project}/manage/tasks`);await page.getByRole("button",{name:"加载会话 JSON",exact:true}).click();const area=page.getByRole("region",{name:"会话 JSON 归档",exact:true});const file=area.getByLabel("选择会话归档",{exact:true});await file.setInputFiles({name:"invalid.json",mimeType:"application/json",buffer:Buffer.from('{"format":"not-a-context"}')});await expect(area.getByRole("alert")).toContainText("不是支持的");await file.setInputFiles({name:"large.json",mimeType:"application/json",buffer:Buffer.alloc(1048577,32)});await expect(area.getByRole("alert")).toContainText("超过 1 MiB");await expect(area.getByRole("button",{name:"确认加载 · 不运行任务",exact:true})).toHaveCount(0);expect(writes).toEqual([]);
});
test("native context archive download preview import and reload preserve inert evidence",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const m=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8"));const b=m.bbox;expect(b.project).toMatch(/^TEST-/);
  const nav=await(await request.get("/api/navigation")).json();const p=nav.items.find((v:{project_id:string})=>v.project_id===b.project);expect(p.conversation_id).toBeTruthy();
  const root=`/api/projects/${b.project}/conversations/${p.conversation_id}`;const before=await(await request.get(`${root}/tasks/${b.task_id}/workspace`)).json();const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});
  await page.goto(`/projects/${b.project}/manage/tasks?task=${b.task_id}`);await page.getByRole("button",{name:"保存会话 JSON",exact:true}).click();const area=page.getByRole("region",{name:"会话 JSON 归档",exact:true});const downloadPromise=page.waitForEvent("download");await area.getByRole("button",{name:/保存会话 JSON/}).click();const download=await downloadPromise;const path=await download.path();expect(path).toBeTruthy();const archive=JSON.parse(readFileSync(path!,"utf8"));expect(archive.format).toBe("annotagent.context");expect(archive.payload.integrity.truncated).toBe(false);
  await area.getByLabel("选择会话归档",{exact:true}).setInputFiles({name:"TEST-context.json",mimeType:"application/json",buffer:Buffer.from(JSON.stringify(archive))});await expect(area.getByRole("heading",{name:"确认加载只读历史"})).toBeVisible();await area.getByRole("button",{name:"确认加载 · 不运行任务",exact:true}).dblclick();const loaded=page.getByRole("region",{name:"已加载会话上下文",exact:true});await expect(loaded).toBeVisible();await expect(loaded).toContainText("继续执行须新建真实任务");const url=page.url();expect(url).toContain("context=");await page.reload();await expect(loaded).toBeVisible();expect(writes.filter(v=>v.endsWith("/context-imports"))).toHaveLength(1);expect(writes.every(v=>v.includes("/context-imports"))).toBe(true);expect(await(await request.get(`${root}/tasks/${b.task_id}/workspace`)).json()).toEqual(before);
  await page.screenshot({path:"/tmp/annotagent-context-archive-TEST.png",fullPage:true});
});
test("native context import recovers lost response without dispatching or repeating POST",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const b=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8")).bbox;expect(b.project).toMatch(/^TEST-/);const nav=await(await request.get("/api/navigation")).json();const p=nav.items.find((v:{project_id:string})=>v.project_id===b.project);const root=`/api/projects/${b.project}`;const exported=await request.get(`${root}/conversations/${p.conversation_id}/context-archive`);expect(exported.ok(),await exported.text()).toBeTruthy();const archive=await exported.json();
  let imports=0;await page.route(`**${root}/context-imports`,async route=>{imports++;const response=await route.fetch();expect(response.ok(),await response.text()).toBeTruthy();await route.abort("failed");});const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});await page.goto(`/projects/${b.project}/manage/tasks`);await page.getByRole("button",{name:"加载会话 JSON",exact:true}).click();await page.getByLabel("选择会话归档",{exact:true}).setInputFiles({name:"TEST-lost-response.json",mimeType:"application/json",buffer:Buffer.from(JSON.stringify(archive))});await page.getByRole("button",{name:"确认加载 · 不运行任务",exact:true}).click();await expect(page.getByRole("region",{name:"已加载会话上下文",exact:true})).toBeVisible();expect(imports).toBe(1);expect(page.url()).toContain("import_command=");await page.reload();await expect(page.getByRole("region",{name:"已加载会话上下文",exact:true})).toBeVisible();expect(imports).toBe(1);expect(writes.every(v=>v.includes("/context-imports"))).toBe(true);
});
test("native task history restores real multi-turn trace on reload without executing",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const m=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8"));const root=`/api/projects/${m.project}/conversations/${m.conversation_id}/tasks/${m.task_id}`;const expected=await(await request.get(`${root}/workspace`)).json();
  const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});const url=`/projects/${m.project}/manage/tasks?task=${m.task_id}`;await page.goto(url);const region=page.getByRole("region",{name:"任务执行轨迹",exact:true});await expect(region.getByRole("heading",{name:/多轮对话/})).toBeVisible();await expect(region).toContainText(`模型调用与决策回执 · ${expected.calls.length}`);await region.locator("summary").filter({hasText:"完整任务工作快照"}).click();await expect(region.locator("pre").last()).toHaveText(JSON.stringify(expected,null,2));await page.reload();await expect(region.getByRole("heading",{name:/多轮对话/})).toBeVisible();expect(writes).toEqual([]);
});
test("native excluded candidate remains inspectable without reappearing in terminal results",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const b=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8")).bbox;expect(b.project).toMatch(/^TEST-/);
  const headers={"x-annotagent-csrf":(await(await request.get("/api/session")).json()).csrf_token};
  // End only TEST pending assistance; exclusion inspection must not impersonate its answer.
  for(const h of await(await request.get(`${b.task_root}/human-requests`)).json())if(h.status==="pending"){const cancelled=await request.post(`${b.task_root}/human-requests/${h.input.id}/cancel`,{headers,data:{}});expect(cancelled.ok(),await cancelled.text()).toBeTruthy();}
  const original=await(await request.get(b.sample_url)).json();const revisions=(await(await request.get(b.feedback_url)).json()).revisions;const previous=[...revisions].reverse().find((r:{outcome_id:string;corrected_value:unknown})=>r.outcome_id===b.candidate_id&&r.corrected_value);const sequence=(revisions.at(-1)?.sequence||0)+1;
  const exclusion={revision_id:randomUUID(),sample_test_id:b.sample_test_id,image_id:b.image_id,sequence,reason:"exclude_target",outcome_id:b.candidate_id,corrected_value:null,corrected_label:null,note:"TEST explicit native exclusion inspection",created_at:new Date().toISOString()};const saved=await request.post(b.feedback_url,{headers,data:exclusion});expect(saved.ok(),await saved.text()).toBeTruthy();
  try{
    const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});await page.goto(`/projects/${b.project}/work?task=${b.task_id}&pane=image&image=${b.image_id}`);const pane=page.getByRole("complementary",{name:"图片与标注",exact:true});const region=pane.getByRole("region",{name:"已排除候选检查",exact:true});await expect(region).toHaveCount(0);await expect(pane.locator(".canvas-region > svg > g > rect")).toHaveCount(0);
    const toggle=pane.locator("summary").filter({hasText:"已排除的样例候选"});await toggle.click();await expect(region.getByLabel("查看已排除候选",{exact:true})).toHaveValue(b.candidate_id);await expect(region).toContainText(exclusion.note);await expect(region.locator("svg image")).toBeVisible();await expect(region.getByRole("button",{name:"Move box with arrow keys",exact:true})).toHaveCount(0);await toggle.click();await page.reload();await toggle.click();await expect(region).toContainText(exclusion.note);expect(writes).toEqual([]);expect(await(await request.get(b.sample_url)).json()).toEqual(original);
  }finally{
    const latest=(await(await request.get(b.feedback_url)).json()).revisions;const restored=await request.post(b.feedback_url,{headers,data:{...exclusion,revision_id:randomUUID(),sequence:latest.at(-1).sequence+1,reason:"poor_boundary",corrected_value:previous?.corrected_value||b.terminal_candidate.outcome.value,corrected_label:previous?.corrected_label||b.terminal_candidate.outcome.label,note:"TEST restore prior visible candidate",created_at:new Date().toISOString()}});expect(restored.ok(),await restored.text()).toBeTruthy();
  }
});
test("native geometry stage view uses saved terminal projection without writes",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");const b=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8")).bbox;
  const before=await(await request.get(b.sample_url)).json();const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});await page.goto(`/projects/${b.project}/work?task=${b.task_id}&pane=image&image=${b.image_id}`);
  const region=page.getByRole("region",{name:"几何阶段对比",exact:true});await expect(region).toHaveCount(0);const toggle=page.locator("summary").filter({hasText:"查看几何阶段对比"});await toggle.click();await expect(region.getByRole("img",{name:"原图上的只读阶段框",exact:true})).toBeVisible();await expect(region).toContainText("精修候选可能已被拒绝");await region.getByRole("checkbox",{name:/终端保留结果/}).uncheck();await expect(region.getByRole("checkbox",{name:/终端保留结果/})).not.toBeChecked();await toggle.click();await expect(region).toHaveCount(0);await page.reload();await expect(region).toHaveCount(0);expect(writes).toEqual([]);expect(await(await request.get(b.sample_url)).json()).toEqual(before);
});
test("native geometry calibration computes an actual TEST detection run without mutating publication",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");
  const manifest=JSON.parse(readFileSync(process.env.AGENT_UI_TEST_MANIFEST!,"utf8"));const b=manifest.bbox;expect(b.project).toMatch(/^TEST-/);
  const headers={"x-annotagent-csrf":(await(await request.get("/api/session")).json()).csrf_token};const root=`/api/projects/${b.project}`;
  const existingRuns=(await(await request.get(`/api/runs?project_id=${b.project}&limit=50`)).json()).runs;
  let runId=existingRuns.find((r:{workflow_version_id:string;status:string})=>r.workflow_version_id===`${b.draft_id}@1`&&["completed","completed_with_review"].includes(r.status))?.id;
  if(!runId){
  const selection={draft_id:b.draft_id,sample_test_id:b.sample_test_id,limit:1};const previewResponse=await request.get(`${root}/processing-preview?${new URLSearchParams({...selection,limit:"1"})}`);expect(previewResponse.ok(),await previewResponse.text()).toBeTruthy();const approval=await previewResponse.json();
  const start=await request.post(`${root}/processing-operations`,{headers,data:{request_id:randomUUID(),selection,expected_revision:approval.revision,authorization_fingerprint:approval.authorization_fingerprint}});expect(start.ok(),await start.text()).toBeTruthy();const processing=await start.json();
  await expect.poll(async()=>(await(await request.get(`/api/batches/${processing.batch_id}`)).json()).batch.status,{timeout:30_000}).toMatch(/^(completed|awaiting_review)$/);
  const batch=(await(await request.get(`/api/batches/${processing.batch_id}`)).json()).batch;runId=batch.child_run_ids[0];
  }
  expect(runId).toBeTruthy();
  // A new Project target deliberately has no matching reviewed evidence; expect insufficient evidence, not accuracy.
  const projectBefore=await(await request.get(`${root}/summary`)).json();if(!projectBefore.project.annotation_schema.some((t:{kind:string;labels:string[]})=>t.kind==="bounding_box"&&t.labels.includes("cup"))){const added=await request.post(`${root}/schema/tasks`,{headers,data:{display_name:"TEST Calibration Target",kind:"bounding_box",labels:["cup"],attributes:{}}});expect(added.ok(),await added.text()).toBeTruthy();}
  const summary=await(await request.get(`${root}/summary`)).json();const version=summary.project.available_workflow_versions.find((v:{workflow_id:string})=>v.workflow_id===b.draft_id);expect(version).toBeTruthy();const frozenUrl=`${root}/workflows/${version.workflow_id}/versions/${version.version}`;const frozen=await(await request.get(frozenUrl)).json();const node=frozen.draft.nodes.find((n:{id:string})=>n.id==="shared.detector");expect(node.model_profile_binding).toBeTruthy();const task=summary.project.annotation_schema.find((t:{kind:string;labels:string[]})=>t.kind==="bounding_box"&&t.labels.includes("cup"));expect(task).toBeTruthy();
  await page.goto(`/projects/${b.project}/manage/pipelines/${version.workflow_id}?version=${version.version}`);await page.locator("summary").filter({hasText:"项目几何策略与校准证据"}).click();const form=page.getByRole("region",{name:"创建几何校准",exact:true});await form.getByLabel("校准标签组",{exact:true}).selectOption(task.id);await form.getByLabel("校准类别",{exact:true}).selectOption("cup");await form.getByLabel("校准模型节点",{exact:true}).selectOption(node.id);await form.getByLabel("校准证据 Run",{exact:true}).fill(runId);await form.getByRole("button",{name:"检查校准范围",exact:true}).click();
  const writes:unknown[]=[];page.on("request",r=>{if(r.method()==="POST"&&new URL(r.url()).pathname===`${root}/geometry-calibrations`)writes.push(r.postDataJSON());});await form.getByRole("button",{name:"确认计算校准",exact:true}).dblclick();await expect(form.getByRole("status")).toContainText("校准记录已保存");expect(writes).toHaveLength(1);
  const records=(await(await request.get(`${root}/geometry-calibrations`)).json()).calibrations;expect(records.length).toBeGreaterThan(0);await expect(form.getByRole("status")).toContainText("uncalibrated · 0 个样本");expect(records.some((c:{report:{key:{task_id:string};status:string;sample_count:number}})=>c.report.key.task_id===task.id&&c.report.status==="uncalibrated"&&c.report.sample_count===0)).toBe(true);await expect(form).toContainText("保存记录不代表校准通过");await page.reload();await page.locator("summary").filter({hasText:"项目几何策略与校准证据"}).click();await expect(form.getByRole("status")).toContainText("校准记录已保存");expect(writes).toHaveLength(1);expect(await(await request.get(frozenUrl)).json()).toEqual(frozen);
});
test("native frozen version restores exact snapshot and rejects missing versions without writes",async({page,request})=>{
  ...
  const geometry=page.getByRole("region",{name:"项目几何质量证据",exact:true});await expect(geometry).toHaveCount(0);await page.locator("summary").filter({hasText:"项目几何策略与校准证据"}).click();await expect(geometry.getByRole("heading",{name:"校准记录",exact:true})).toBeVisible();await expect(geometry).toContainText("不代表当前版本已校准");const calibration=page.getByRole("region",{name:"创建几何校准",exact:true});await expect(calibration).toContainText("没有带 Model Profile 快照的几何输出节点");await expect(calibration.getByRole("button",{name:"检查校准范围",exact:true})).toBeDisabled();const actual=await(await request.get(`/api/projects/${project}/geometry-calibrations`)).json();if(!actual.calibrations.length)await expect(geometry).toContainText("尚无校准记录");await geometry.getByRole("button",{name:"刷新几何证据",exact:true}).click();await expect(geometry.getByRole("heading",{name:"校准记录",exact:true})).toBeVisible();
...
test("controlled mask response fixture renders exact RLE pixels and original toggle removes overlay",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");
  const run=fixtureManifest().run_id;const endpoint=`/api/runs/${run}/pipeline-artifacts`;
...
test("native Artifact deep link shows its actual input image and refresh remains passive",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");
  const run=fixtureManifest().run_id;
...
test("native static validation checks the saved revision without execution and invalidates evidence on edits",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");
  const project=fixtureManifest().project;
  ...
  expect(writes.map(url=>new URL(url).pathname)).toEqual([`/api/workflow-drafts/${draft.id}/validate`]);
...
test("native Pipeline step dialog preserves invalid input then saves only a cloned TEST draft",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");
  const project=fixtureManifest().project;
  const summary=await(await request.get(`/api/projects/${project}/summary`)).json();const version=summary.project.available_workflow_versions[0];
  const frozen=await(await request.get(`/api/projects/${project}/workflows/${version.workflow_id}/versions/${version.version}`)).json();const original=frozen.draft;expect(original.label_pipeline).toBeTruthy();
  const session=await(await request.get("/api/session")).json();const clone=await request.post(`/api/workflows/${version.workflow_id}/versions/${version.version}/clone`,{headers:{"x-annotagent-csrf":session.csrf_token},data:{command_id:randomUUID(),project_id:project,source_snapshot_hash:frozen.content_hash}});expect(clone.ok(),await clone.text()).toBeTruthy();const draft=await clone.json();
  const writes:string[]=[];page.on("request",r=>{if(r.method()!=="GET")writes.push(r.url());});
  await page.goto(`/projects/${project}/manage/pipelines/${draft.id}`);
  ...
  expect(writes.map(url=>new URL(url).pathname)).toEqual([`/api/workflow-drafts/${draft.id}`]);
  const after=await(await request.get(`/api/projects/${project}/workflows/${version.workflow_id}/versions/${version.version}`)).json();expect(after).toEqual(frozen);
});
test("native workflow editor saves an exact TEST draft and never publishes or runs",async({page,request})=>{
  expect((await request.get("/api/health")).headers()["x-annotagent-fixture"]).toBe("external-model-only");
  const projects=await(await request.get("/api/projects")).json();const project=projects.projects.find((p:{id:string})=>p.id===fixtureManifest().project);expect(project).toBeTruthy();
...
test("native model CAS keeps a stale editor without overwriting the winning writer",async({page,request})=>{
  ...
  await region.locator('input[placeholder="名称、模型 ID 或能力"]').fill(name);await region.getByRole("button",{name:"编辑配置",exact:true}).click();
