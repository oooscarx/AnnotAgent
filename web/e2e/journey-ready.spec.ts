...
test("ready fixture journey plans without image calls then authorizes a bounded sandbox test", async ({ page, request }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Save sample feedback", exact: true }).click();
  await expect(page.getByText("Sample feedback saved", { exact: true })).toBeVisible();
  const originalDraft = (await (await request.get(`/api/workflow-drafts?project_id=${projectId}`)).json()).drafts.find((draft: {id:string}) => draft.id === draftId);
  await page.getByRole("button", { name: "Review an adjustment using saved feedback", exact: true }).click();
  ...
  expect(preRevisionSessions.sessions).toHaveLength(1);
  await page.getByRole("checkbox", { name: "I authorize this feedback and planning scope", exact: true }).check();
  await page.getByRole("button", { name: "Authorize plan adjustment", exact: true }).click();
  await expect(page.getByRole("button", { name: "Review revised sample scope", exact: true })).toBeVisible({ timeout: 60_000 });
  expect(sampleRequests).toHaveLength(1);
  await page.reload();
  await expect(page.getByRole("button", { name: "Review revised sample scope", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Review revised sample scope", exact: true }).click();
  await expect(page.getByRole("region", { name: "Sample authorization", exact: true })).toBeVisible();
  await page.getByRole("checkbox", { name: "I reviewed the sample scope" }).check();
  await page.getByRole("button", { name: "Test samples", exact: true }).click();
  await expect(page.getByRole("button", { name: "Before adjustment", exact: true })).toBeVisible();
  ...
  const originalSampleUrl = page.url();
  const copyBeforeEdit = (await (await request.get(`/api/workflow-drafts?project_id=${projectId}`)).json()).drafts.find((draft: {id:string}) => draft.id === copyId);
  const changedCopy = await request.patch(`/api/workflow-drafts/${copyId}`, { headers: { "if-match": String(copyBeforeEdit.revision) }, data: { ...copyBeforeEdit, name: `${copyBeforeEdit.name} edited after test` } });
  expect(changedCopy.ok()).toBeTruthy();
  await page.goto(revisedSampleUrl);
  ...
  await page.goto(originalSampleUrl);
  const afterRevision = (await (await request.get(`/api/workflow-drafts?project_id=${projectId}`)).json()).drafts.find((draft: {id:string}) => draft.id === draftId);
  expect(afterRevision).toEqual(originalDraft);
  expect(sampleRequests).toHaveLength(2);
