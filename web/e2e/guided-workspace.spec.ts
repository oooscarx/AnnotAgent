...
test("open Run Artifact from history without entering an ID", async ({ page }) => {
  ...
  await expect(page.locator(".journey-result-image .annotation-canvas image")).toBeVisible();
  await expect(page.locator(".sidebar, .run-view-tabs, .run-result-metrics, .run-needs-attention, .focus-project-menu")).toHaveCount(0);
  ...
  await page.getByRole("button", { name: `Execution details · ${runId.slice(0, 8)}`, exact: true }).click();
...
test("Project-owned routes preserve hierarchy, resolve owners, and expose Batch detail", async ({ page }) => {
  ...
  await expect(page.locator(".batch-image-runs, .focus-project-menu")).toHaveCount(0);
  await page.getByRole("button", { name: "Show original", exact: true }).click();
  await expect(page).toHaveURL(/display=original/);
  await page.reload();
  await expect(page.getByRole("button", { name: "Show results", exact: true })).toHaveAttribute("aria-pressed", "true");
  await page.getByLabel("Show images", { exact: true }).selectOption("failed");
  await expect(page).toHaveURL(/status=failed/);
  await page.reload();
  await expect(page.getByLabel("Show images", { exact: true })).toHaveValue("failed");
  await expect(page.getByText("failed · provider unavailable", { exact: true })).toBeVisible();
  await page.goto(`/projects/${projectId}/batches/${batchId}?view=history`);
...
test("geometry safety is visible from Results through Improve Automation", async ({ page, request }) => {
  ...
  await expect(page.getByText("Some boundaries have not been verified. A model confidence score is not measured boundary accuracy.", { exact: true })).toBeVisible();
  await expect(page.getByLabel("Semantic and geometry quality")).toHaveCount(0);
  await page.goto(`/projects/${cropProjectId}/runs`);
  await page.getByRole("button", { name: `Execution details · ${fixture!.run.id.slice(0, 8)}`, exact: true }).click();
...
`,
  ...
  await expect(page.locator(".journey-result-image .annotation-shape")).toHaveCount(1);
  await expect(page.getByText("Fallbacks", { exact: true })).toHaveCount(0);
  await expect(page.getByText("Cache hits", { exact: true })).toHaveCount(0);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBeTruthy();
  ...
  await page.reload();
  await expect(page.locator(".journey-result-image .annotation-canvas image")).toBeVisible();
  await expect(page.locator(".journey-result-image .annotation-shape")).toHaveCount(0);
  await expect(page.getByText("No target was found. This is not evidence that the image contains none; inspect the original image.")).toBeVisible();
...
test("primary journey controls, focus, and annotation alternatives work from the keyboard", async ({ page, request }) => {
  ...
  const listToggle = page.getByRole("button", { name: /Annotation list/ });
  await listToggle.focus();
  await page.keyboard.press("Enter");
  const annotationList = page.getByRole("list", { name: "Annotations on canvas" });
  ...
  const selectedResultUrl = page.url();
  await page.reload();
  await expect(page).toHaveURL(selectedResultUrl);
  await page.getByRole("button", { name: /Annotation list/ }).click();
  await expect(page.getByRole("list", { name: "Annotations on canvas" }).getByRole("button", { pressed: true })).toHaveCount(1);
