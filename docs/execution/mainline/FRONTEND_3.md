...
- The existing opt-in HTTP smoke also passed on `fdeaa94`, but its retained trace still used the legacy queryful Journey preview followed by an explicit `/execution` POST. It therefore does **not** prove the new upload → parameterless preview → one consent → automatic Sample path. `P0-F3-006` asks Backend for that exact clean-SHA HTTP/SQLite/external-TEST-Provider trace, including restart recovery; A1 remains blocked for end-to-end acceptance until it exists.
