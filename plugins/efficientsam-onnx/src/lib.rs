...
use annotagent_core::{
    ...
    VisionBackendTimings, VisionBackendUsage, VisionCapability,
};
...
const MODEL_ID: &str = "efficientsam-ti-onnx";
const ENCODER_FILENAME: &str = "efficientsam_ti_encoder.onnx";
const DECODER_FILENAME: &str = "efficientsam_ti_decoder.onnx";
...
const MAX_PROMPT_POINTS: usize = 6;
const MAX_IMAGE_BYTES: usize = 32 * 1024 * 1024;
const MAX_CACHE_VALUES: usize = 8 * 1024 * 1024;
const CACHE_MAGIC: &[u8; 8] = b"AAESEM1\0";

pub struct EfficientSamOnnxPlugin {
    manifest: PluginManifest,
    loaded: RwLock<Option<LoadedEfficientSam>>,
    embeddings: RwLock<HashMap<String, Arc<CachedEmbedding>>>,
}

struct LoadedEfficientSam {
...
struct EfficientSamImageTransform {
    source_width: u32,
    source_height: u32,
    resized_width: u32,
    resized_height: u32,
...
impl EfficientSamOnnxPlugin {
    ...
    async fn embedding(
    ...
    ) -> Result<(Arc<CachedEmbedding>, EfficientSamImageTransform, bool, u64), PluginSdkError> {
        ...
        if let Some(cached) = self
            ...
            .cloned()
        {
            return Ok((cached, transform, true, 0));
        }
        let cache_path = cache_dir
            .join("efficientsam-embeddings")
        ...
        let input = NamedTensor::float32("batched_images", tensor);
        ...
        let encoder_duration_ms = duration_ms(outputs.duration);
        let tensor = outputs
            .tensors
            .iter()
        ...
        self.embeddings
            ...
            .map_err(|_| PluginSdkError::Plugin("embedding cache lock was poisoned".to_owned()))?
            .insert(cache_key, Arc::clone(&cached));
        Ok((cached, transform, false, encoder_duration_ms))
    }
}

#[async_trait]
impl ExpertModelPlugin for EfficientSamOnnxPlugin {
    async fn setup(&self, context: PluginRuntimeContext) -> Result<(), PluginSdkError> {
        ...
        std::fs::create_dir_all(context.cache_dir.join("efficientsam-embeddings"))?;
        *self
            .loaded
            .write()
            .map_err(|_| PluginSdkError::Plugin("model state lock was poisoned".to_owned()))? =
            Some(LoadedEfficientSam {
    ...
    async fn warmup(&self, model_id: &str, context: WarmupContext) -> Result<(), PluginSdkError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let encoder_input = NamedTensor {
                name: "batched_images".to_owned(),
                shape: vec![1, 3, 256, 256],
                data: TensorData::Float32(vec![0.0; 3 * 256 * 256]),
            };
    ...
    async fn infer(
        ...
        {
            return Err(PluginSdkError::Plugin(
                "request does not match the EfficientSAM model capability".to_owned(),
            ));
        }
        if context.cancellation.is_cancelled() {
            return Err(PluginSdkError::Plugin("request cancelled".to_owned()));
        ...
        let (embedding, transform, cache_hit, encoder_duration_ms) = self
            .embedding(
                &image,
                &image_digest,
                &context.cache_dir,
        ...
        let masks = match prompts {
            PromptInput::Boxes(prompts) => {
                ...
                let mut decoder_duration_ms = 0_u64;
                for prompt in &prompts.prompts {
                    let (prompt_masks, decoder_run_ms) = run_prompt(
                        Arc::clone(&decoder),
                        Arc::clone(&embedding),
                    ...
                    decoder_duration_ms = decoder_duration_ms.saturating_add(decoder_run_ms);
                }
                (prompts.reference.clone(), masks, decoder_duration_ms)
            }
        ...
        let artifact = MaskSetArtifact {
            ...
            metadata: BTreeMap::from([
                ...
                (
                    "encoder_inference_ms".to_owned(),
                    serde_json::json!(encoder_duration_ms),
                ),
                (
                    "decoder_inference_ms".to_owned(),
                    serde_json::json!(masks.2),
                ),
                ("runtime".to_owned(), serde_json::json!("rust-onnx-cpu")),
            ]),
        };
        artifact.validate().map_err(PluginSdkError::Plugin)?;
        let inference_ms = encoder_duration_ms.saturating_add(masks.2);
        ...
        Ok(PipelineInferenceResponse {
            ...
            metadata: BTreeMap::from([
                (
                    "plugin_id".to_owned(),
                    serde_json::json!("org.annotagent.efficientsam-onnx"),
                ),
                (
                    "contract".to_owned(),
                    serde_json::json!("efficientsam-ti-split-onnx-v1"),
                ),
            ]),
            usage: VisionBackendUsage {
                source: Some("rust-onnx-cpu".to_owned()),
                compute_milliseconds: Some(inference_ms),
...
async fn run_prompt(
    ...
    transform: EfficientSamImageTransform,
    options: DecodeOptions,
    cancellation: &tokio_util::sync::CancellationToken,
) -> Result<(Vec<MaskArtifactItem>, u64), PluginSdkError> {
    if cancellation.is_cancelled() {
        return Err(PluginSdkError::Plugin("request cancelled".to_owned()));
    }
    ...
    let decoder_duration_ms = duration_ms(outputs.duration);
    Ok((
...
fn preprocess(
    image: &DynamicImage,
) -> Result<(TensorF32, EfficientSamImageTransform), PluginSdkError> {
    ...
    // EfficientSAM's exported encoder performs its own 1024x1024 resize and
    // ImageNet normalization. Keep native geometry unless a bounded downscale
    // is required, and only convert host pixels to RGB float32 in [0, 1].
    let scale = (ENCODER_SIZE as f32 / source_width.max(source_height) as f32).min(1.0);
    ...
    let resized = if (resized_width, resized_height) == (source_width, source_height) {
        image.to_rgb8()
    } else {
        image
            .resize_exact(
    ...
    for (x, y, pixel) in resized.enumerate_pixels() {
        let index = y as usize * resized_width as usize + x as usize;
        for channel in 0..3 {
            values[channel * plane + index] = f32::from(pixel[channel]) / 255.0;
        }
    }
    Ok((
        TensorF32::new(
...
fn decoder_inputs(
    ...
    prompt: PromptSpec<'_>,
    transform: EfficientSamImageTransform,
) -> Result<Vec<NamedTensor>, PluginSdkError> {
    let (coordinates, labels) = match prompt {
        ...
        PromptSpec::Point(prompt) => {
            if prompt.points.is_empty() || prompt.points.len() > MAX_PROMPT_POINTS {
                return Err(PluginSdkError::Plugin(format!(
                    "EfficientSAM point prompts require between 1 and {MAX_PROMPT_POINTS} points"
                )));
            }
            let mut coordinates = Vec::with_capacity(prompt.points.len() * 2);
            let mut labels = Vec::with_capacity(prompt.points.len());
    ...
    Ok(vec![
        ...
        NamedTensor {
            name: "batched_point_coords".to_owned(),
            shape: vec![1, 1, point_count, 2],
            data: TensorData::Float32(coordinates),
        },
        NamedTensor {
            name: "batched_point_labels".to_owned(),
            shape: vec![1, 1, point_count],
            data: TensorData::Float32(labels),
        },
        NamedTensor {
            name: "orig_im_size".to_owned(),
            shape: vec![2],
            data: TensorData::Int64(vec![
...
impl EfficientSamImageTransform {
    fn x_scale(self) -> f32 {
...
fn decode_masks(
    ...
    let masks = outputs
        .iter()
        .find(|tensor| tensor.name == "output_masks")
        .ok_or_else(|| PluginSdkError::Plugin("decoder did not return output_masks".to_owned()))?;
    let scores = outputs
        .iter()
    ...
    let (mask_count, mask_height, mask_width) = match masks.shape.as_slice() {
        [1, 1, count, height, width] | [1, count, height, width] | [count, height, width] => {
            (*count, *height, *width)
        }
        _ => {
            return Err(PluginSdkError::Plugin(format!(
    ...
    if mask_count == 0
        ...
        || score_values[..mask_count]
            .iter()
            .any(|score| !score.is_finite())
    {
        return Err(PluginSdkError::Plugin(
            "mask and score tensors are empty, non-finite, or do not match their shapes".to_owned(),
        ));
    }
...
fn one_image_artifact(
    ...
    match images.as_slice() {
        ...
        _ => Err(PluginSdkError::Plugin(
            "EfficientSAM requires exactly one Image Artifact".to_owned(),
        )),
    }
}
...
fn one_prompt_input(request: &PipelineInferenceRequest) -> Result<PromptInput<'_>, PluginSdkError> {
    ...
    if prompts.len() != 1 {
        return Err(PluginSdkError::Plugin(
            "EfficientSAM requires exactly one BoxPromptSet or PointPromptSet".to_owned(),
        ));
    }
...
fn validate_encoder_contract(session: &OnnxSession) -> Result<(), PluginSdkError> {
    ...
    if descriptor.inputs.len() != 1
        || descriptor.inputs[0].name != "batched_images"
        || descriptor.inputs[0].element_type != "f32"
        || descriptor.inputs[0].shape != [-1, 3, -1, -1]
        || descriptor.outputs.len() != 1
        || descriptor.outputs[0].name != "image_embeddings"
        || descriptor.outputs[0].element_type != "f32"
        || descriptor.outputs[0].shape != [-1, EMBEDDING_CHANNELS_I64, -1, -1]
    {
        return Err(PluginSdkError::Plugin(format!(
            "checkpoint does not match the EfficientSAM image encoder contract: inputs={:?}, outputs={:?}",
...
fn validate_decoder_contract(session: &OnnxSession) -> Result<(), PluginSdkError> {
    let descriptor = session.descriptor();
    let matches = |name: &str, element_type: &str, shape: &[i64]| {
        descriptor.inputs.iter().any(|input| {
            input.name == name && input.element_type == element_type && input.shape == shape
        })
    };
    let output_matches = |name: &str, shape: &[i64]| {
        descriptor.outputs.iter().any(|output| {
            output.name == name && output.element_type == "f32" && output.shape == shape
        })
    };
    if descriptor.inputs.len() != 4
        || !matches("image_embeddings", "f32", &[-1, 256, 64, 64])
        || !matches("batched_point_coords", "f32", &[1, 1, -1, 2])
        || !matches("batched_point_labels", "f32", &[1, 1, -1])
        || !matches("orig_im_size", "i64", &[2])
        || descriptor.outputs.len() != 3
        || !output_matches("output_masks", &[-1, -1, -1, -1, -1])
        || !output_matches("iou_predictions", &[1, 1, -1])
        || !output_matches("onnx::Shape_1830", &[-1, -1, -1, -1])
    {
        return Err(PluginSdkError::Plugin(format!(
            "checkpoint does not match the EfficientSAM mask decoder contract: inputs={:?}, outputs={:?}",
            descriptor.inputs, descriptor.outputs
        )));
    }
    Ok(())
...
fn validate_embedding(embedding: &CachedEmbedding) -> Result<(), PluginSdkError> {
    ...
    {
        return Err(PluginSdkError::Plugin(
            "cached embedding does not match the EfficientSAM contract".to_owned(),
        ));
    }
    Ok(())
}
...
fn warmup_decoder_inputs() -> Vec<NamedTensor> {
    vec![
        ...
        NamedTensor {
            name: "orig_im_size".to_owned(),
            shape: vec![2],
            data: TensorData::Int64(vec![256, 256]),
        },
    ]
}
...
mod tests {
    ...
    fn transform() -> EfficientSamImageTransform {
        EfficientSamImageTransform {
            source_width: 4,
            source_height: 2,
            resized_width: 4,
    ...
    fn manifest_declares_two_weight_components_and_only_prompted_segmentation() {
        let plugin = EfficientSamOnnxPlugin::load().expect("plugin");
        ...
        let recipe_model: annotagent_plugin_api::PluginModelManifest = serde_json::from_str(
            include_str!("../../../model-recipes/efficientsam-ti/plugin-model.json"),
        )
        .expect("recipe model contract");
        assert_eq!(plugin.descriptor_model(), &recipe_model);
        assert_eq!(
            format!(
                "{:x}",
                Sha256::digest(serde_json::to_vec(&recipe_model).expect("canonical model JSON"))
            ),
    ...
    #[test]
    fn preprocess_matches_efficientsam_export_contract() {
        let image = DynamicImage::ImageRgb8(RgbImage::from_pixel(4, 2, Rgb([255, 128, 0])));
        let (tensor, transform) = preprocess(&image).expect("preprocess");
        assert_eq!(tensor.shape, [1, 3, 2, 4]);
        assert!((transform.x_scale() - 1.0).abs() < f32::EPSILON);
        assert!((transform.y_scale() - 1.0).abs() < f32::EPSILON);
        assert!((tensor.values[0] - 1.0).abs() < f32::EPSILON);
        assert!((tensor.values[8] - 128.0 / 255.0).abs() < f32::EPSILON);
        assert!(tensor.values[16].abs() < f32::EPSILON);
    }
    ...
    fn point_and_box_prompts_preserve_sam_labels() {
        ...
        let box_inputs =
            decoder_inputs(&embedding, PromptSpec::Box(&bbox), transform()).expect("box inputs");
        assert_eq!(box_inputs[1].shape, [1, 1, 2, 2]);
        ...
        let point_inputs = decoder_inputs(&embedding, PromptSpec::Point(&point), transform())
            .expect("point inputs");
        assert_eq!(point_inputs[1].shape, [1, 1, 1, 2]);
        assert_eq!(point_inputs[2].data, TensorData::Float32(vec![1.0]));
        assert_eq!(point_inputs[3].data, TensorData::Int64(vec![2, 4]));
    }
