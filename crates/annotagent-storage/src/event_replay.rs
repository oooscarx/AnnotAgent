...
impl SqliteStore {
    ...
    pub fn run_events_after(&self, run: RunId, cursor: Option<&str>, limit: u32) -> Result<Option<Vec<RunEvent>>, StorageError> {
...
mod tests {
    use super::*;
    use annotagent_core::{ProjectId,RunStatus,RunEventKind,RunEventPayload};
    use annotagent_runtime::{RuntimeStore,RunRecord};
    #[tokio::test]
    async fn replay_is_ordered_bounded_owned_and_survives_restart() {
        ...
        let run=RunId::new();
        store.create_run(&RunRecord{id:run,project_id:ProjectId::new(),project_name:"TEST".into(),skill_id:"TEST".into(),provider:"mock".into(),model:"mock".into(),status:RunStatus::Pending,project_schema_json:"{}".into(),workflow_snapshot_json:None}).await.unwrap();
        let mut ids=Vec::new();
        for _ in 0..3 {
            let event=RunEvent::new(run,RunEventKind::RunCreated,RunEventPayload::State{from:None,to:RunStatus::Pending,reason:None});
            ids.push(event.event_id.to_string());
            store.record_event(&event).await.unwrap();
        }
        assert_eq!(store.run_events_after(run,None,2).unwrap().unwrap().len(),2);
        drop(store);
        let store=SqliteStore::open(&path).unwrap();
        let page=store.run_events_after(run,Some(&ids[1]),2).unwrap().unwrap();
        assert_eq!(page.len(),1);
        assert_eq!(page[0].event_id.to_string(),ids[2]);
        assert!(store.run_events_after(RunId::new(),Some(&ids[1]),2).unwrap().is_none());
        assert!(store.run_events_after(run,Some("TEST-missing"),2).unwrap().is_none());
    }
}
