...
pub use conversation_image_class::{
    ConversationImageClassAction, ConversationImageClassAnswerInput,
    ConversationImageClassCreateInput, ConversationImageClassReview, ConversationImageClassScope,
    ConversationImageClassStatus, conversation_image_class_effective_outcome,
};
...
impl SqliteStore {
    ...
    pub fn migrate(&self) -> Result<(), StorageError> {
        self.with_connection(|connection| {
            ...
            transaction.execute_batch(include_str!("../../../migrations/0033_conversation_sample_assistance.sql"))?;
            transaction.execute("INSERT OR IGNORE INTO schema_migrations(version,name,applied_at) VALUES(33,'conversation_sample_assistance',?1)",[Utc::now().to_rfc3339()])?;
            ...
            let has_owner_slug_batch_lifecycle = transaction.query_row(
                "SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version=75)",
                [],
                |row| row.get::<_, bool>(0),
            )?;
            if !has_owner_slug_batch_lifecycle {
                crate::batch::migrate_delivery_review_batch_lifecycle(&transaction)?;
                transaction.execute(
                    "INSERT INTO schema_migrations(version,name,applied_at) VALUES(75,'delivery_review_owner_slug_batch_lifecycle',?1)",
                    [Utc::now().to_rfc3339()],
                )?;
            }
