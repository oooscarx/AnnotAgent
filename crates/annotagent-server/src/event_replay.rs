...
pub(super) fn gap(run: RunId) -> ApiError {
    ApiError {
        ...
        body: json!({"status":409,"code":"event_cursor_gap","error":"Event cursor is absent from this Run; reload the snapshot","suggested_action":"reload_snapshot","snapshot_url":format!("/api/runs/{run}"),"history_url":format!("/api/runs/{run}/events")}),
    }
}
pub(super) async fn events(
    ...
    let Some(run) = query.run_id else {
        if cursor.is_some() {
            return Err(ApiError::bad_request(
                "Reconnect requires exact run_id scope",
            ));
        }
        return super::events(
            State(state),
    ...
    let permit = state
        .security
        .try_acquire_sse()
        .ok_or_else(|| ApiError::too_many_requests("SSE client limit reached"))?;
    let first = state
        .application
        .store()
        .run_events_after(run, cursor.as_deref(), 100)
    ...
    let stream=stream::unfold((state,cursor,std::collections::VecDeque::from(first),permit,false),move |(state,mut cursor,mut pending,permit,done)| async move {
        ...
        loop {
            ...
            if let Ok(Some(events)) = state.application.store().run_events_after(run,cursor.as_deref(),100) {
...
mod tests {
    ...
    async fn sse_reconnect_replays_exact_run_and_reports_cursor_gap() {
        ...
        let response = request(
            ...
            &format!("/api/events?run_id={run}&last_event_id={}", first.event_id),
            None,
        )
        .await;
        assert_eq!(response.status(), StatusCode::OK);
        let mut body = response.into_body().into_data_stream();
        let bytes = tokio::time::timeout(Duration::from_secs(2), body.next())
            .await
            .unwrap()
            .unwrap()
            .unwrap();
        ...
        let response = request(
            &service,
            axum::http::Method::GET,
            &format!("/api/events?run_id={run}&last_event_id=TEST-missing"),
            None,
        )
        .await;
