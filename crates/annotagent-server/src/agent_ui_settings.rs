...
pub(super) struct View { view: Option<String> }
...
pub(super) fn conflict(current: &str) -> ApiError {
    ApiError { status: StatusCode::CONFLICT, body: json!({"error":"Settings changed; reload before updating future budgets","status":409,"code":"settings_revision_conflict","current_revision":current,"suggested_action":"reload_settings"}) }
}
...
pub(super) async fn get(State(state): State<ServerState>, Query(view): Query<View>) -> ApiResult<Json<Value>> {
...
pub(super) async fn patch(State(state): State<ServerState>, Json(input): Json<BudgetPatch>) -> ApiResult<Json<Value>> {
    let _write = state.settings_writes.lock().await;
    let mut settings=state.settings.read().await.clone();
    let current=revision(&settings);
    if input.expected_revision != current { return Err(conflict(&current)); }
...
mod tests {
    ...
    async fn safe_settings_are_passive_and_budget_patch_rejects_stale_revision() {
        ...
        let initial=response_json(request(&service,axum::http::Method::GET,"/api/settings?view=agent-ui",None).await).await;
        assert!(initial["revision"].is_string());
        assert!(initial["sections"]["providers"].is_object());
        assert!(!initial.to_string().contains(&dir.path().display().to_string()));
        assert!(initial.get("provider").is_none());
        let input=json!({"expected_revision":initial["revision"],"budget":{"max_requests":7}});
        let saved=response_json(request(&service,axum::http::Method::PATCH,"/api/settings",Some(input.clone())).await).await;
        assert_eq!(saved["sections"]["usage_budget"]["future_run_budget"]["max_requests"],7);
        let stale=request(&service,axum::http::Method::PATCH,"/api/settings",Some(input)).await;
